...
# ================================================================================
# [RENDER-FIX] HEALTH CHECK HTTP SERVER
# Render exige qu'un service web réponde sur $PORT pour le health check.
# Ce mini-serveur HTTP tourne dans un thread daemon séparé.
# ================================================================================

class _HealthHandler(BaseHTTPRequestHandler):
    """Handler minimal — répond 200 OK à tout GET pour le health check Render."""
    def do_GET(self):
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.end_headers()
        self.wfile.write(b'{"status":"ok","service":"world_scanner"}')
    def log_message(self, format, *args):
        pass  # Silence les logs HTTP

def _start_health_server():
    """Lance le serveur HTTP health check sur $PORT (Render l'impose)."""
    port = int(os.environ.get("PORT", 8080))
    try:
        server = HTTPServer(("0.0.0.0", port), _HealthHandler)
        logger.info("[RENDER] Health server démarré sur port %d", port)
        server.serve_forever()
    except Exception as e:
        logger.warning("[RENDER] Health server erreur: %s", e)
...
async def main():
    # [RENDER-FIX] Démarrer health check HTTP AVANT la boucle principale
    # Render tue le service si aucune réponse HTTP dans les 60s au démarrage
