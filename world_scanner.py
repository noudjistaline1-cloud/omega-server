# WORLD SCANNER MICRO2 ADAPTIVE — symbol-aware online source learning — V139 SESSION CONTEXT ONLY
# ================================================================================
...
try:
    from SMART_HOUR_ENGINE import smart_hour_decision, _load_stats10y
    _SHE_AVAILABLE = True
...
def _is_london_dst(now_utc: datetime) -> bool:
    """[FIX-DST] Londres ouvre 07h UTC en été (BST, fin mars→fin oct.) et 08h
    UTC en hiver (GMT). SESSIONS["LONDON"] ci-dessus est fixé à 07h en dur —
    correct seulement ~7 mois/an. Cohérent avec le fix déjà appliqué côté EA
    (IsLondonDST dans STALINE_PNEUMA_130). Vérifié babypips.com/learn/forex-
    trading-sessions le 19/07/2026."""
    year = now_utc.year
    # dernier dimanche de mars 01:00 UTC → dernier dimanche d'octobre 01:00 UTC
    def last_sunday(y, m):
        next_month = datetime(y+1, 1, 1, tzinfo=timezone.utc) if m == 12 \
                     else datetime(y, m+1, 1, tzinfo=timezone.utc)
        last_day = next_month - timedelta(days=1)
        offset = (last_day.weekday() - 6) % 7  # weekday(): Mon=0..Sun=6
        return last_day - timedelta(days=offset)
    start = last_sunday(year, 3).replace(hour=1, minute=0, second=0, microsecond=0)
    end   = last_sunday(year, 10).replace(hour=1, minute=0, second=0, microsecond=0)
    return start <= now_utc < end


def _is_ny_dst(now_utc: datetime) -> bool:
    """[FIX-DST] New York : 2e dimanche mars 07:00 UTC → 1er dimanche nov.
    06:00 UTC (EDT). SESSIONS["NEW_YORK"] est fixé à 12h en dur (= convention
    été EDT uniquement) — décalé d'1h le reste de l'année."""
    year = now_utc.year
    def nth_sunday(y, m, n):
        d = datetime(y, m, 1, tzinfo=timezone.utc)
        first_sunday = d + timedelta(days=(6 - d.weekday()) % 7)
        return first_sunday + timedelta(weeks=n - 1)
    start = nth_sunday(year, 3, 2).replace(hour=7, minute=0, second=0, microsecond=0)
    end   = nth_sunday(year, 11, 1).replace(hour=6, minute=0, second=0, microsecond=0)
    return start <= now_utc < end


def get_active_sessions(hour_utc: int) -> List[str]:
    """Retourne les sessions réellement actives en UTC, avec DST dynamique.

    OVERLAP_LN_NY est dérivé de LONDON ∩ NEW_YORK au lieu d'une fenêtre
    indépendante : cela évite le décalage d'une heure pendant les semaines où
    les changements DST US/UK ne sont pas synchronisés.
    """
    now = datetime.now(timezone.utc)
    london_shift = 1 if not _is_london_dst(now) else 0
    ny_shift = 1 if not _is_ny_dst(now) else 0
    ...
    active = []
    # Sydney/Tokyo/London/New York utilisent leurs horaires locaux convertis en UTC.
    if _active(SESSIONS["SYDNEY"]["start"], SESSIONS["SYDNEY"]["end"]):
        active.append("SYDNEY")
    if _active(SESSIONS["TOKYO"]["start"], SESSIONS["TOKYO"]["end"]):
        active.append("TOKYO")

    london_active = _active(SESSIONS["LONDON"]["start"] + london_shift,
                            SESSIONS["LONDON"]["end"] + london_shift)
    ny_active = _active(SESSIONS["NEW_YORK"]["start"] + ny_shift,
                        SESSIONS["NEW_YORK"]["end"] + ny_shift)
...
def get_session_bias_score(sym: str, hour_utc: int) -> Tuple[float, str, float, float]:
    ...
    Statistique descriptive uniquement; camera_allowed n'est jamais déterminé par la session.
...
SCALP_MODE_CONDITIONS = {
    # Session is CONTEXT, never a hard entry veto.
    # Volatility/news still affect execution quality and sizing, not session eligibility.
    "vix_max":        20.0,
    "news_score_max":  0.25,
    "fg_range":       (35, 65),
}
...
def get_trade_mode(
    ...
    """
    Détermine SCALP/SWING/BOTH sans jamais utiliser la session comme veto.

    Architecture V139 :
      SESSION = contexte de liquidité/volatilité/exécution.
      SESSION != condition d'autorisation.

    La session peut donc modifier tp/sl/lot/temps de détention, mais une heure
    historiquement faible ne transforme jamais un signal exploitable en WAIT.
    """
    if hour_utc < 0:
        hour_utc = datetime.now(timezone.utc).hour

    session_score, session_name, vol_mult, session_wr = get_session_bias_score(sym, hour_utc)
    abs_score = abs(score)
    ...
    # VIX : qualité du scalping, pas interdiction générale de trader.
    if vix < SCALP_MODE_CONDITIONS["vix_max"]:
        scalp_ok = True
        scalp_reasons.append(f"VIX={vix:.1f} favorable au scalp")
    else:
        scalp_reasons.append(f"VIX={vix:.1f} élevé → scalp plus sélectif")

    # La session est informative : jamais de `scalp_ok=False` parce qu'une
    # heure n'appartient pas à une liste historique.
    active = get_active_sessions(hour_utc)
    scalp_reasons.append(
        f"Session={session_name} active={','.join(active)} vol×{vol_mult:.2f} — contexte seulement"
    )

    # News : on peut préférer swing/tenir plus longtemps, mais ne pas confondre
    # événement macro et fermeture forcée. Les vrais hard-gates restent côté serveur.
    if abs(news_score) > SCALP_MODE_CONDITIONS["news_score_max"]:
        scalp_ok = False
        scalp_reasons.append(f"News fort={news_score:.2f} → scalp désactivé, swing privilégié")

    # Faible liquidité : réduire l'agressivité, pas interdire le trade.
    if vol_mult < 0.60:
        scalp_reasons.append(f"Liquidité historique faible vol×{vol_mult:.2f} → sizing/exécution prudents")
    elif vol_mult > 1.20:
        scalp_reasons.append(f"Liquidité/activité élevée vol×{vol_mult:.2f} → fenêtre dynamique")

    # SWING : direction + conviction, indépendant de la session.
    ...
    if abs(news_score) >= SWING_MODE_CONDITIONS["news_score_min"]:
        swing_ok = True
        swing_reasons.append(f"News fort={news_score:+.2f} → swing possible")
    ...
    # Si le marché a un signal exploitable mais aucune configuration stricte
    # SCALP/SWING, on conserve un mode ADAPTIVE au lieu de WAIT pour raison horaire.
    if swing_ok and scalp_ok:
        ...
        mode = "SCALP"
    elif abs_score >= 0.12 and conviction >= 0.30:
        mode = "ADAPTIVE"
        scalp_reasons.append("Signal exploitable hors critères stricts → mode ADAPTIVE")
    else:
        mode = "WAIT"

    # Session influence le risque/exécution de façon continue, sans veto.
    session_risk_mult = max(0.70, min(1.15, 0.90 + (vol_mult - 1.0) * 0.20))
    tp_mult = 1.0
    sl_mult = 1.0
    lot_mult = session_risk_mult
    max_hold_min = 30

    if mode == "SCALP":
        tp_mult = 1.5 * max(0.80, min(1.35, vol_mult))
        sl_mult = 1.0
        max_hold_min = 10
    elif mode == "SWING":
        tp_mult = 3.5 * min(max(0.80, vol_mult), 1.5)
        sl_mult = 1.5
        lot_mult *= 0.70
        max_hold_min = 240
    elif mode == "BOTH":
        tp_mult = 2.5 * min(max(0.80, vol_mult), 1.3)
        sl_mult = 1.2
        lot_mult *= 0.85
        max_hold_min = 60
    elif mode == "ADAPTIVE":
        tp_mult = 2.0 * min(max(0.80, vol_mult), 1.35)
        sl_mult = 1.15
        max_hold_min = 90
    else:
        lot_mult = 0.0
    ...
    return {
        ...
        "session_bias": round(session_score, 4),
        "session_wr_buy": round(session_wr, 4),
        "session_is_context_only": True,
        "session_veto": False,
        "session_risk_mult": round(session_risk_mult, 3),
        "scalp_ok": scalp_ok,
        "swing_ok": swing_ok,
...
# ================================================================================
# [MICRO-1] MICROSTRUCTURE OHLCV-ONLY — VPIN/BVC + Amihud Illiquidity
# Fonctionne sur tout actif (BTC/XAU/XAG/forex) avec juste OHLC+volume déjà
# récupéré via Yahoo Finance dans fetch_technicals_for_symbol — zéro appel
# réseau supplémentaire. Sources : Easley/López de Prado/O'Hara (VPIN/BVC),
# Amihud (2002) illiquidity ratio. Note honnête : VPIN/BVC est un signal
# parmi d'autres (une source seule ne doit jamais dominer la cascade) —
# corrélé à la volatilité, pas une preuve absolue de toxicité du flux.
# ================================================================================

def _normal_cdf(x: float) -> float:
    """Φ(x) — CDF de la loi normale standard, approximation Abramowitz-Stegun."""
...
def _calc_vpin_bvc(opens: list, closes: list, volumes: list,
                    n_buckets: int = 20) -> dict:
    """
    VPIN via Bulk Volume Classification (Easley/López de Prado/O'Hara).
    Classe la fraction "achat" de chaque bougie via la CDF normale du
    rendement standardisé (pas besoin du sens trade-par-trade) :
        V_buy  = Volume × Φ( (Close - Open) / σ_ΔP )
        V_sell = Volume - V_buy
        VPIN   = moyenne mobile sur N buckets de |V_buy - V_sell| / V_total
    Retourne un score signé [-1, +1] (signe = déséquilibre net achat/vente
    sur la fenêtre récente) + le VPIN brut [0,1] (intensité, sans direction).
    """
    n = min(len(opens), len(closes), len(volumes))
    if n < max(10, n_buckets):
        return {"ok": False, "vpin": 0.0, "vpin_score": 0.0, "vpin_regime": "N/A"}

    opens, closes, volumes = opens[-n_buckets*3:], closes[-n_buckets*3:], volumes[-n_buckets*3:]
    deltas = [closes[i] - opens[i] for i in range(len(closes))]
    mean_d = sum(deltas) / len(deltas)
    var_d  = sum((d - mean_d) ** 2 for d in deltas) / max(1, len(deltas) - 1)
    sigma  = math.sqrt(var_d) if var_d > 0 else 1e-9

    buckets = []
    net_imbalance = 0.0
    total_vol_all = 0.0
    for i in range(len(closes)):
        vol = max(0.0, float(volumes[i] or 0.0))
        if vol <= 0:
            continue
        z = (closes[i] - opens[i]) / sigma
        v_buy  = vol * _normal_cdf(z)
        v_sell = vol - v_buy
        buckets.append(abs(v_buy - v_sell) / vol if vol > 0 else 0.0)
        net_imbalance += (v_buy - v_sell)
        total_vol_all += vol

    if not buckets or total_vol_all <= 0:
        return {"ok": False, "vpin": 0.0, "vpin_score": 0.0, "vpin_regime": "N/A"}

    recent = buckets[-n_buckets:] if len(buckets) >= n_buckets else buckets
    vpin = sum(recent) / len(recent)

    # Direction : imbalance net (achat/vente) normalisé par le volume total,
    # amorti par l'intensité VPIN (un VPIN élevé mais équilibré ne dit rien).
    net_dir = max(-1.0, min(1.0, net_imbalance / total_vol_all))
    vpin_score = max(-1.0, min(1.0, net_dir * min(1.0, vpin / 0.7)))

    regime = "DANGER_UNILATERAL" if vpin > 0.7 else ("ELEVATED" if vpin > 0.4 else "BALANCED")
    return {"ok": True, "vpin": round(vpin, 4), "vpin_score": round(vpin_score, 4),
            "vpin_regime": regime}


def _calc_amihud(closes: list, volumes: list, window: int = 20) -> dict:
    """
    Amihud (2002) illiquidity ratio : |return| / volume_notionnel sur une
    fenêtre. Un ratio élevé et croissant = le prix bouge beaucoup pour peu
    de volume réel derrière (mouvement fragile, vide de liquidité, risque
    de reversal violent). Rôle : moduler la conviction, jamais la direction.
    """
    n = min(len(closes), len(volumes))
    if n < window + 1:
        return {"ok": False, "amihud": 0.0, "amihud_score": 0.0, "fragile": False}

    closes, volumes = closes[-(window + 1):], volumes[-(window + 1):]
    ratios = []
    for i in range(1, len(closes)):
        vol = max(1e-9, float(volumes[i] or 0.0))
        notional = vol * closes[i]
        if notional <= 0 or closes[i - 1] <= 0:
            continue
        ret = abs((closes[i] - closes[i - 1]) / closes[i - 1])
        ratios.append(ret / notional * 1e6)  # mise à l'échelle lisible

    if len(ratios) < window // 2:
        return {"ok": False, "amihud": 0.0, "amihud_score": 0.0, "fragile": False}

    avg_ratio  = sum(ratios) / len(ratios)
    recent_avg = sum(ratios[-max(3, window // 4):]) / max(3, window // 4)
    spike   = (recent_avg / avg_ratio) if avg_ratio > 0 else 1.0
    fragile = spike > 1.8
    amihud_score = -min(1.0, max(0.0, (spike - 1.0) / 2.0)) if fragile else 0.0

    return {"ok": True, "amihud": round(avg_ratio, 4), "amihud_spike": round(spike, 2),
            "amihud_score": round(amihud_score, 4), "fragile": fragile}
...
async def fetch_technicals_for_symbol(
                ...
                try:
                    async with session.get(use_url,
                        ...
                        if r.status == 200:
                            ...
                            vols_raw   = q.get("volume", [])
                            # aligne les 3 séries en ne gardant que les indices où close existe
                            _valid_idx = [i for i, x in enumerate(closes_raw) if x is not None]
                            closes = [closes_raw[i] for i in _valid_idx]
                            if len(closes) < 20:
                                continue
                            opens  = [opens_raw[i] if i < len(opens_raw) and opens_raw[i] is not None
                                      else closes[j] for j, i in enumerate(_valid_idx)]
                            vols   = [vols_raw[i] if i < len(vols_raw) and vols_raw[i] is not None
                                      else 0.0 for i in _valid_idx]
                            ...
                            # [MICRO-1] VPIN/BVC + Amihud — OHLCV déjà en main, zéro coût réseau
                            _vpin   = _calc_vpin_bvc(opens, closes, vols)
                            _amihud = _calc_amihud(closes, vols)
                            tech["vpin_score"]   = _vpin.get("vpin_score", 0.0)
                            tech["vpin_regime"]  = _vpin.get("vpin_regime", "N/A")
                            tech["vpin_ok"]      = _vpin.get("ok", False)
                            tech["amihud_score"] = _amihud.get("amihud_score", 0.0)
                            tech["amihud_fragile"] = _amihud.get("fragile", False)

                            # [MICRO-2] candles brutes 1d pour fetch_wyckoff_phases
                            # (qui attend une liste de dicts open/high/low/close)
                            if tf == "1d":
                                highs_raw = q.get("high", [])
                                lows_raw  = q.get("low",  [])
                                highs = [highs_raw[i] if i < len(highs_raw) and highs_raw[i] is not None
                                         else closes[j] for j, i in enumerate(_valid_idx)]
                                lows  = [lows_raw[i]  if i < len(lows_raw)  and lows_raw[i]  is not None
                                         else closes[j] for j, i in enumerate(_valid_idx)]
                                tech["ohlc_1d"] = [
    ...
    # [MICRO-1] VPIN/Amihud de référence : 1h préféré (horizon naturel du VPIN),
    # sinon 1d, sinon 5m. amihud_fragile = OR sur tous les TF (une fragilité
    # détectée sur n'importe quel horizon reste un signal d'alerte valable).
    _ref_tf   = tf_results.get("1h", tf_results.get("1d", tf_results.get("5m", {})))
    _vpin_ref = _ref_tf.get("vpin_score", 0.0) if _ref_tf.get("vpin_ok") else 0.0
    _amihud_fragile_any = any(d.get("amihud_fragile", False) for d in tf_results.values())
    _amihud_ref = min((d.get("amihud_score", 0.0) for d in tf_results.values()
                       if d.get("amihud_score", 0.0) != 0.0), default=0.0)
    ...
    return {
        ...
        "by_tf":     {tf: {"score": d["score"], "rsi": d["rsi"], "signal": d["signal"],
                            "vpin_score": d.get("vpin_score", 0.0),
                            "amihud_score": d.get("amihud_score", 0.0)}
                      for tf, d in tf_results.items()},
        "price":     tf_results.get("5m", tf_results.get("1h", {})).get("price", 0),
        "vpin_score":     round(_vpin_ref, 4),
        "vpin_regime":    _ref_tf.get("vpin_regime", "N/A"),
        "amihud_score":   round(_amihud_ref, 4),
        "amihud_fragile": _amihud_fragile_any,
        "ohlc_1d":        tf_results.get("1d", {}).get("ohlc_1d", []),
    }
...
RSS_SOURCES = [
    ...
    # [FIX-RSS-DEAD] rss.reuters.com n'existe plus (DNS mort, testé 19/07/2026,
    # code 000 = connexion impossible) — Reuters a fermé ses flux RSS publics.
    # Remplacé par WSJ Markets (feeds.a.dj.com), qualité institutionnelle
    # comparable, testé et fonctionnel (200 OK, contenu vérifié en direct).
    {"url": "https://feeds.a.dj.com/rss/RSSMarketsMain.xml",    "name": "WSJ_Markets",   "w": 1.4, "cat": "eco"},
    {"url": "https://www.cnbc.com/id/100003114/device/rss/rss.html", "name": "CNBC_Top", "w": 1.3, "cat": "geo"},
    ...
    # [FIX-RSS-DEAD] feeds.feedburner.com/forexfactory/news → 404 confirmé.
    # Remplacé par Investing.com Stock Market News, testé fonctionnel.
    {"url": "https://www.investing.com/rss/news_25.rss",        "name": "Investing_Stocks", "w": 1.2, "cat": "eco"},
    # [FIX-RSS-DEAD] ft.com/rss/home/uk → bloqué (403) quel que soit le
    # User-Agent testé — semble activement anti-bot, pas juste discontinué.
    # Remplacé par MarketWatch (via le flux Dow Jones officiel, plus fiable
    # que l'URL directe marketwatch.com/rss qui elle aussi renvoie 403).
    {"url": "https://feeds.content.dowjones.io/public/rss/mw_topstories", "name": "MarketWatch", "w": 1.3, "cat": "eco"},
...
async def fetch_fred_real_yield(session: aiohttp.ClientSession) -> Dict:
    """
    [NOUVEAU] Taux réel 10 ans (TIPS) via FRED — série DFII10.
    C'est le facteur fondamental le plus établi pour l'or, au-delà du seul
    DXY : taux réel EN BAISSE = haussier pour l'or (le coût d'opportunité de
    détenir un actif sans rendement diminue), taux réel EN HAUSSE = baissier
    pour l'or. Absent du scanner jusqu'ici alors que le taux Fed nominal
    (fetch_fred_rate) y était déjà — cette donnée réelle manquait.
    """
    result = {"real_yield_10y": 1.80, "ok": False, "source": "fallback"}
    for headers in [H_BOT, H_BROWSER]:
        try:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10),
                ...
                if r.status == 200:
                    ...
                    lines = [l for l in text.strip().split("\n")
                             if "," in l and not l.startswith("DATE")]
                    # DFII10 peut avoir des valeurs "." certains jours fériés —
                    # on remonte jusqu'à trouver la dernière valeur réelle.
                    for line in reversed(lines):
                        parts = line.split(",")
                        if len(parts) >= 2 and parts[1].strip() not in (".", ""):
                            result = {
                                "real_yield_10y": round(float(parts[1].strip()), 3),
                                "real_yield_date": parts[0].strip(),
                                "ok":       True,
                                "source":   "FRED",
                            }
                            return result
...
async def fetch_cot_report(session: aiohttp.ClientSession, cftc_code: str = "088691") -> Dict:
    """
    [NOUVEAU] Vraie donnée COT (Commitment of Traders) via l'API publique
    Socrata de la CFTC (publicreporting.cftc.gov) — remplace le proxy
    complètement figé qui existait côté serveur (+0.55 codé en dur pour l'or,
    jamais mis à jour depuis sa création).
    cftc_code="088691" = Gold COMEX (code CFTC standard pour l'or).
    [LIMITE HONNÊTE] Je n'ai pas d'accès web dans cette session pour vérifier
    en direct le schéma exact de cette API (noms de champs, dataset ID) — ce
    code est écrit de façon défensive (.get() partout, repli sûr) pour ne
    jamais planter même si le format a changé, mais DOIT être vérifié avec un
    vrai test après déploiement (regarder les logs [COT-FETCH] au démarrage).
    """
            ...
            if r.status == 200:
                ...
                if data and isinstance(data, list) and len(data) > 0:
                    ...
                    result = {
                        ...
                        "managed_money_net": int(long_mm - short_mm),
                        "report_date": row.get("report_date_as_yyyy_mm_dd", ""),
                        "ok": True,
                        "source": "CFTC",
                    }
    except Exception as _e_cot:
        logger.debug(f"[COT-FETCH] échec (repli sur défaut): {_e_cot}")
    return result


async def fetch_yield_curve(session: aiohttp.ClientSession) -> Dict:
    """
    [NOUVEAU] Spread taux 10 ans - 2 ans (FRED, série T10Y2Y déjà calculée).
    Un des meilleurs indicateurs avancés de récession établis en finance —
    l'or performe historiquement très bien dans les 12-18 mois précédant/
    pendant une récession, indépendamment du seul niveau des taux réels.
    Spread négatif (inversion) = signal de récession à venir = généralement
    favorable à l'or à moyen terme.
    """
    result = {"yield_curve_10y2y": 0.50, "ok": False, "source": "fallback"}
    for headers in [H_BOT, H_BROWSER]:
        try:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10),
                ...
                if r.status == 200:
                    ...
                    for line in reversed(lines):
                        ...
                        if len(parts) >= 2 and parts[1].strip() not in (".", ""):
                            result = {
                                "yield_curve_10y2y": round(float(parts[1].strip()), 3),
                                "yield_curve_date": parts[0].strip(),
                                "ok": True,
                                "source": "FRED",
                            }
                            return result
...
def compute_gold_seasonality(month: int = None) -> Dict:
    """
    [NOUVEAU] Saisonnalité calendaire de l'or — patron établi de demande
    physique, zéro donnée externe nécessaire :
    - Oct-Déc : saison des mariages en Inde (forte demande physique)
    - Avr-Mai : Akshaya Tritiya (achat d'or traditionnel en Inde)
    - Jan-Fév : Nouvel An lunaire chinois (demande cadeaux/investissement)
    Retourne un score [-0.3..+0.3] à intégrer comme facteur modeste,
    pas un signal dominant — la demande physique influence le prix mais
    beaucoup plus lentement/faiblement que les flux ETF ou le COT.
    """
    if month is None:
        month = datetime.now(timezone.utc).month
    seasonal_bias = {
        1: +0.15, 2: +0.15,               # Nouvel An chinois
        3: 0.0,
        4: +0.20, 5: +0.20,               # Akshaya Tritiya
        6: 0.0, 7: 0.0, 8: 0.0, 9: 0.0,
        10: +0.25, 11: +0.30, 12: +0.20,  # Saison des mariages en Inde
    }
    bias = seasonal_bias.get(month, 0.0)
    return {"gold_seasonal_bias": bias, "month": month,
            "reason": "saisonnalité demande physique (Inde/Chine)" if bias != 0 else "période neutre"}


async def fetch_imf_gold_reserves(session: aiohttp.ClientSession,
                                    countries: List[str] = None) -> Dict:
    """
    [NOUVEAU] Réserves d'or des banques centrales via l'API publique du FMI
    (data.imf.org, système SDDS) — MENSUEL, pas trimestriel comme le World
    Gold Council. Suit les principaux acheteurs connus (Chine, Russie,
    Pologne, Inde, Turquie) pour détecter une accumulation soutenue qui crée
    un plancher structurel sur le prix.
    [LIMITE HONNÊTE] Schéma d'API du FMI pas vérifiable en direct dans cette
    session (pas d'accès web) — écrit défensivement, à vérifier après
    déploiement via les logs [IMF-GOLD-FETCH].
    """
    if countries is None:
        countries = ["CN", "RU", "PL", "IN", "TR"]  # codes ISO2 — principaux acheteurs connus
    result = {"reserves_trend": "UNKNOWN", "countries_data": {}, "ok": False, "source": "fallback"}
    try:
        codes = "+".join(countries)
        url = (f"https://www.imf.org/external/datamapper/api/v1/reserves_gold"
               f"?periods=2024,2025,2026")
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as r:
            if r.status == 200:
                data = await r.json(content_type=None)
                values = data.get("values", {}).get("reserves_gold", {})
                increasing = 0
                for c in countries:
                    series = values.get(c, {})
                    if series and len(series) >= 2:
                        vals = sorted(series.items())
                        if float(vals[-1][1]) > float(vals[-2][1]):
                            increasing += 1
                        result["countries_data"][c] = vals[-1][1]
                if len(result["countries_data"]) > 0:
                    result["reserves_trend"] = "ACCUMULATION" if increasing >= len(result["countries_data"]) / 2 else "STABLE"
                    result["ok"] = True
                    result["source"] = "IMF"
    except Exception as _e_imf:
        logger.debug(f"[IMF-GOLD-FETCH] échec (repli sur défaut): {_e_imf}")
    return result


_comex_stocks_prev: Dict = {"registered_total": None, "date": None}

async def fetch_comex_warehouse_stocks(session: aiohttp.ClientSession) -> Dict:
    """
    [NOUVEAU-COMPLET] Stocks physiques d'or dans les entrepôts COMEX (CME) —
    quotidien, gratuit, proxy de tension sur le marché physique. Une baisse
    rapide des stocks "registered" (disponibles pour livraison) pendant que
    l'open interest reste élevé signale une pression de livraison physique
    réelle — historiquement corrélé à des mouvements haussiers sur l'or.

    Parsing réel : le fichier CME peut être en .xls (ancien format BIFF,
    nécessite xlrd) ou .xlsx (nécessite openpyxl) — on tente les deux. Les
    en-têtes de colonnes exactes ne sont pas garanties identiques d'un jour à
    l'autre ni vérifiables sans accès live, donc on cherche par correspondance
    partielle insensible à la casse ("regist"/"eligib") plutôt que des noms de
    colonnes figés — plus robuste aux petites variations de mise en page.

    [LIMITE HONNÊTE] Toujours pas testé contre le vrai fichier en direct
    (pas d'accès web dans cette session). Écrit défensivement : si le format
    a changé ou si le parsing échoue pour toute raison, repli sûr sans
    plantage — vérifie les logs [COMEX-STOCKS] après déploiement pour
    confirmer que ça fonctionne réellement.
    """
    global _comex_stocks_prev
    result = {"registered_change_pct": 0.0, "registered_total": None,
              "ok": False, "source": "fallback"}
    try:
        ...
        if df is None or len(df) == 0:
            logger.debug("[COMEX-STOCKS] fichier reçu mais illisible par pandas (xlrd/openpyxl)")
            return result

        # Trouver la ligne d'en-tête réelle : celle qui contient une cellule
        # ressemblant à "registered" (le fichier CME a souvent quelques lignes
        # de titre/date avant le vrai tableau).
        header_row_idx = None
        registered_col = None
        eligible_col = None
        ...
        if header_row_idx is None or registered_col is None:
            logger.debug("[COMEX-STOCKS] en-tête 'Registered' introuvable dans le fichier")
            return result

        # Sommer les valeurs numériques de la colonne "Registered" sous l'en-tête
        data_rows = df.iloc[header_row_idx + 1:]
        registered_vals = pd.to_numeric(data_rows.iloc[:, registered_col], errors="coerce").dropna()
        registered_total = float(registered_vals.sum()) if len(registered_vals) > 0 else None

        eligible_total = None
        if eligible_col is not None:
            eligible_vals = pd.to_numeric(data_rows.iloc[:, eligible_col], errors="coerce").dropna()
            eligible_total = float(eligible_vals.sum()) if len(eligible_vals) > 0 else None

        if registered_total is None or registered_total <= 0:
            logger.debug("[COMEX-STOCKS] colonne Registered trouvée mais aucune valeur numérique exploitable")
            return result

        # Variation jour après jour par rapport au dernier fetch réussi
        change_pct = 0.0
        if _comex_stocks_prev.get("registered_total"):
            prev = _comex_stocks_prev["registered_total"]
            if prev > 0:
                change_pct = round((registered_total - prev) / prev * 100.0, 2)

        _comex_stocks_prev = {"registered_total": registered_total,
                               "date": datetime.now(timezone.utc).isoformat()}

        result = {
            "registered_total": round(registered_total, 0),
            "eligible_total": round(eligible_total, 0) if eligible_total else None,
            "registered_change_pct": change_pct,
            "ok": True,
            "source": "CME",
        }
        logger.info(f"[COMEX-STOCKS] Registered={registered_total:.0f} "
                    f"(Δ{change_pct:+.2f}%) Eligible={eligible_total}")
    except Exception as _e_cme:
        logger.debug(f"[COMEX-STOCKS] échec (repli sur défaut): {_e_cme}")
    return result
...
# [V27-ADD] Fichier séparé pour les co-occurrences de paires de sources
# (sert uniquement à la pénalité d'indépendance ci-dessous) — séparé du
# fichier existant pour ne rien modifier de l'état déjà en production.
SOURCE_PAIR_FILE             = "source_pair_state.json"
SOURCE_PAIR_MAX_SAMPLES      = 100
...
# [V27-ADD] {asset_type: {"srcA|srcB": [agree_bool, ...]}} — pour chaque
# évaluation où srcA ET srcB avaient toutes les deux un score non-neutre,
# True si elles pointaient dans le même sens (peu importe si correct ou
# non — c'est une mesure d'accord entre sources, pas de justesse).
_SOURCE_PAIR_AGREE = {}
...
def _source_pair_load():
    global _SOURCE_PAIR_AGREE
    try:
        with open(SOURCE_PAIR_FILE, "r") as f:
            _SOURCE_PAIR_AGREE = json.load(f)
    except Exception:
        _SOURCE_PAIR_AGREE = {}


def _source_pair_save():
    try:
        with open(SOURCE_PAIR_FILE, "w") as f:
            json.dump(_SOURCE_PAIR_AGREE, f)
    except Exception:
        pass


def classify_regime(tech: Dict, vix: float) -> str:
    """[V28-ADD] Classification de régime simple à partir de données déjà
    calculées par fetch_technicals_for_symbol() — AUCUN nouvel appel réseau.
    Retourne un régime parmi : TREND, RANGE, HIGH_VOL, LOW_VOL, BREAKOUT,
    UNKNOWN. Volontairement simple (pas de ML) : c'est un classifieur de
    contexte pour pondérer les sources, pas un signal de trade.
    - HIGH_VOL / LOW_VOL priment sur TREND/RANGE quand le VIX est extrême,
      car la volatilité globale du marché change la fiabilité de toutes
      les sources, quel que soit le comportement du prix lui-même.
    - BREAKOUT : le prix du jour dépasse le plus haut/bas des N-1 jours
      précédents (ohlc_1d), signe d'une sortie de range.
    - TREND : momentum absolu au-dessus d'un seuil ET prix aligné avec les
      deux moyennes mobiles (SMA20/50 du même côté).
    - RANGE : le reste — momentum faible, pas de cassure.
    """
    try:
        if vix >= 28.0:
            return "HIGH_VOL"
        ...
        ohlc = tech.get("ohlc_1d") or []
        by_tf = tech.get("by_tf", {})
        d1h = by_tf.get("1h", {})
        momentum_pct = tech.get("_momentum_pct", None)

        if len(ohlc) >= 5:
            closes = [c["close"] for c in ohlc if c.get("close")]
            highs  = [c["high"]  for c in ohlc if c.get("high")]
            lows   = [c["low"]   for c in ohlc if c.get("low")]
            if closes and highs and lows:
                last_close   = closes[-1]
                prior_high   = max(highs[-11:-1]) if len(highs) >= 11 else max(highs[:-1])
                prior_low    = min(lows[-11:-1])  if len(lows)  >= 11 else min(lows[:-1])
                if last_close > prior_high or last_close < prior_low:
                    return "BREAKOUT"
                # ATR simplifié (moyenne des true ranges) / prix -> proxy de vol locale
                trs = [max(highs[i], closes[i-1]) - min(lows[i], closes[i-1])
                       for i in range(1, len(closes))]
                atr_pct = (sum(trs) / len(trs)) / last_close * 100 if trs and last_close else 0.0
                if atr_pct >= 3.5:
                    return "HIGH_VOL"
                if atr_pct <= 0.6:
                    return "LOW_VOL"

        rsi_1h = d1h.get("rsi", 50.0)
        sc_1h  = d1h.get("score", 0.0)
        if abs(sc_1h) >= 0.35 and (rsi_1h >= 58 or rsi_1h <= 42):
            return "TREND"
        return "RANGE"
    except Exception:
        return "UNKNOWN"
...
# ================================================================================
# [V28-ADD] CALIBRATION PAR BUCKETS — pas de Platt/isotonic paramétrique.
# Justification (vérifiée) : Platt scaling a besoin d'un jeu de validation
# de taille correcte pour ne pas overfitter, et isotonic regression n'est
# fiable qu'à partir d'environ 1000 points. Les échantillons ici (n=50-100)
# sont bien en dessous — un modèle paramétrique inventerait de la précision
# qui n'existe pas. La calibration par buckets est plus grossière mais ne
# peut pas mentir au-delà de ce que les données montrent vraiment, et
# chaque bucket a son propre garde-fou d'échantillon minimum.
# ================================================================================
CALIB_FILE          = "calibration_state.json"
CALIB_MIN_PER_BUCKET = 20     # sous ce seuil -> bucket non fiable, pas utilisé
CALIB_BUCKETS = [(0.50, 0.55), (0.55, 0.60), (0.60, 0.70), (0.70, 0.80), (0.80, 1.01)]
_CALIB_STORE: Dict[str, Dict[str, List[bool]]] = {}   # {asset_type: {bucket_label: [correct,...]}}


def _calib_bucket_label(conf: float) -> Optional[str]:
...
def _calib_save():
    try:
        with open(CALIB_FILE, "w") as f:
            json.dump(_CALIB_STORE, f)
    except Exception:
        pass


def record_calibration_outcome(asset_type: str, raw_confidence: float, was_correct: bool):
    """À appeler quand une prédiction (avec sa confiance brute) arrive à
    échéance et qu'on sait si elle était correcte — alimente les buckets."""
    label = _calib_bucket_label(max(0.0, min(0.999, raw_confidence)))
    if label is None:
        return
    bucket = _CALIB_STORE.setdefault(asset_type, {}).setdefault(label, [])
    bucket.append(bool(was_correct))
    if len(bucket) > 300:
        del bucket[:len(bucket) - 300]
    _calib_save()


def calibrate_confidence(asset_type: str, raw_confidence: float) -> Dict:
    """Retourne la confiance calibrée SI le bucket a assez d'échantillons
    (>=CALIB_MIN_PER_BUCKET), sinon renvoie la confiance brute inchangée —
    jamais de calibration inventée sur un échantillon insuffisant."""
    label = _calib_bucket_label(max(0.0, min(0.999, raw_confidence)))
    if label is None:
        return {"calibrated": raw_confidence, "used_calibration": False, "bucket_n": 0}
    lst = _CALIB_STORE.get(asset_type, {}).get(label, [])
    n = len(lst)
    if n < CALIB_MIN_PER_BUCKET:
        return {"calibrated": raw_confidence, "used_calibration": False, "bucket_n": n}
    real_rate = sum(1 for x in lst if x) / n
    return {"calibrated": round(real_rate, 4), "used_calibration": True, "bucket_n": n}


# ================================================================================
# [V28-ADD] DÉTECTION DE DRIFT — CUSUM sur le flux d'accuracy de chaque source.
# Principe (Page, 1954, même famille que le CUSUM déjà utilisé côté EA pour
# le drift de prix) : cumul des écarts à la moyenne de référence ; si le
# cumul dépasse un seuil h, le comportement a changé -> on le signale et on
# réduit la confiance accordée à cette source, sans attendre que
# _source_accuracy() l'ait "digéré" sur 100 échantillons.
# ================================================================================
CUSUM_K = 0.05     # tolérance (slack) autour de la référence 0.5
CUSUM_H = 5.0      # seuil de déclenchement
_CUSUM_STATE: Dict[str, Dict[str, float]] = {}   # {"asset:source": {"pos":..,"neg":..}}


def update_drift_cusum(asset_type: str, source: str, was_correct: bool) -> bool:
    """Met à jour le CUSUM pour (asset_type, source) avec un nouveau résultat
    (1.0 si correct, 0.0 sinon), retourne True si un drift est détecté."""
    key = f"{asset_type}:{source}"
    st = _CUSUM_STATE.setdefault(key, {"pos": 0.0, "neg": 0.0})
    x = 1.0 if was_correct else 0.0
    dev = x - 0.5
    st["pos"] = max(0.0, st["pos"] + dev - CUSUM_K)
    st["neg"] = max(0.0, st["neg"] - dev - CUSUM_K)
    drift = st["pos"] > CUSUM_H or st["neg"] > CUSUM_H
    if drift:
        st["pos"] = 0.0; st["neg"] = 0.0   # reset après détection
    return drift


def is_drifting(asset_type: str, source: str) -> bool:
    key = f"{asset_type}:{source}"
    st = _CUSUM_STATE.get(key)
    if not st:
        return False
    return st["pos"] > CUSUM_H * 0.6 or st["neg"] > CUSUM_H * 0.6   # alerte précoce (60% du seuil)
...
def record_pending_scores(sym: str, asset_type: str, price: float, scores: Dict[str, float],
    ...
    [V28-ADD] regime/confidence sont optionnels — un appelant qui ne les
    fournit pas garde exactement l'ancien comportement (regime="UNKNOWN"
    n'est simplement jamais utilisé comme clé préférentielle par
    _source_accuracy tant que le bucket régime réel n'existe pas)."""
    if price <= 0:
        return
...
def evaluate_pending_scores(current_prices: Dict[str, float]):
    ...
    for entry in _PENDING_EVAL:
        ...
        perf_key = f"{asset_type}:{entry.get('sym','')}" if entry.get("sym") else asset_type
        bucket = _SOURCE_PERF.setdefault(perf_key, {})
        actual_dir = "BUY" if moved_up else "SELL"
        bucket_dir = _SOURCE_PERF.setdefault(f"{perf_key}:{actual_dir}", {})
        # [V28-ADD] Sous-compartiment régime+direction — même principe que
        # BUY/SELL : s'ajoute, ne remplace rien.
        regime = entry.get("regime", "UNKNOWN")
        bucket_regime = None
        if regime and regime != "UNKNOWN":
            bucket_regime = _SOURCE_PERF.setdefault(f"{perf_key}:{actual_dir}:{regime}", {})
        ...
        for src, sc in entry["scores"].items():
            ...
            lst_dir = bucket_dir.setdefault(src, [])
            lst_dir.append(bool(correct))
            if len(lst_dir) > SOURCE_PERF_MAX_SAMPLES:
                del lst_dir[:len(lst_dir) - SOURCE_PERF_MAX_SAMPLES]
            if bucket_regime is not None:
                lst_reg = bucket_regime.setdefault(src, [])
                lst_reg.append(bool(correct))
                if len(lst_reg) > SOURCE_PERF_MAX_SAMPLES:
                    del lst_reg[:len(lst_reg) - SOURCE_PERF_MAX_SAMPLES]
            updated = True
            nonneutral_srcs.append((src, predicted_up))

            # [V28-ADD] CUSUM drift par source
            try:
                if update_drift_cusum(asset_type, src, correct):
                    drift_flags.append((asset_type, src))
            except Exception:
                pass

        # [V28-ADD] Calibration : la confiance globale du cycle (si fournie)
        # est comparée au résultat réel du symbole (direction majoritaire).
        conf = entry.get("confidence", 0.0)
        if conf and conf > 0:
            overall_predicted_up = sum(1 for _, up in nonneutral_srcs if up) >= (len(nonneutral_srcs) / 2.0) if nonneutral_srcs else None
            if overall_predicted_up is not None:
                overall_correct = (overall_predicted_up and moved_up) or (not overall_predicted_up and moved_down)
                try:
                    record_calibration_outcome(asset_type, conf, overall_correct)
                except Exception:
                    pass

        if len(nonneutral_srcs) >= 2:
            pair_bucket = _SOURCE_PAIR_AGREE.setdefault(asset_type, {})
            for i in range(len(nonneutral_srcs)):
                for j in range(i + 1, len(nonneutral_srcs)):
                    s1, d1 = nonneutral_srcs[i]
                    s2, d2 = nonneutral_srcs[j]
                    pair_key = "|".join(sorted([s1, s2]))
                    lst_p = pair_bucket.setdefault(pair_key, [])
                    lst_p.append(bool(d1 == d2))
                    if len(lst_p) > SOURCE_PAIR_MAX_SAMPLES:
                        del lst_p[:len(lst_p) - SOURCE_PAIR_MAX_SAMPLES]
                    pair_updated = True
    ...
    if pair_updated:
        _source_pair_save()
    for at, src in drift_flags:
        logger.warning("[DRIFT-CUSUM] %s/%s : changement de comportement détecté — confiance réduite temporairement", at, src)


def _source_accuracy(asset_type: str, source: str, symbol: str = "", direction: str = "", regime: str = "") -> float:
    """Accuracy glissante avec shrinkage Beta + pondération exponentielle récente.
    [V28] Ordre de préférence : symbole+direction+régime -> symbole+direction
    -> symbole -> global. Chaque niveau retombe sur le suivant si vide.
    Aucun paramètre optionnel fourni = comportement 100% identique à avant."""
    keys = []
    if symbol and direction and regime and regime != "UNKNOWN":
        keys.append(f"{asset_type}:{symbol}:{direction}:{regime}")
    if symbol and direction:
        keys.append(f"{asset_type}:{symbol}:{direction}")
    if symbol:
        keys.append(f"{asset_type}:{symbol}")
    keys.append(asset_type)
    lst = []
    for k in keys:
        cand = _SOURCE_PERF.get(k, {}).get(source, [])
        if cand:
            lst = cand; break
    n=len(lst)
    if n < 5: return 0.5
    ...
    raw=sum(w*(1.0 if ok else 0.0) for w,ok in zip(weights,lst))/den
    prior_n=min(20,n)
    shrunk=(raw*n + 0.5*prior_n)/(n+prior_n)
    return max(0.20,min(0.80,shrunk))


def _pair_independence_factor(asset_type: str, src: str, other_srcs: List[str]) -> float:
    """Facteur d'indépendance de `src` par rapport aux autres sources actives
    dans le même calcul (0.5..1.0). 1.0 = aucune corrélation détectée (ou pas
    assez de données -> neutre, ne pénalise jamais par manque de preuve)."""
    pair_bucket = _SOURCE_PAIR_AGREE.get(asset_type, {})
    worst = 1.0
    for other in other_srcs:
        if other == src:
            continue
        pair_key = "|".join(sorted([src, other]))
        lst = pair_bucket.get(pair_key, [])
        if len(lst) < 10:
            continue
        agree_rate = sum(1 for x in lst if x) / len(lst)
        factor = 1.0 - max(0.0, (agree_rate - 0.5)) * 1.0
        worst = min(worst, factor)
    return max(0.5, worst)


def compute_weights_adaptive(vix: float, asset_type: str = "forex", urgency: bool = False,
                              symbol: str = "", regime: str = "") -> Dict[str, float]:
    """compute_weights() de base, ajusté par : (1) accuracy récente
    [éventuellement régime-conditionnelle], (2) indépendance entre sources
    corrélées, (3) pénalité de drift CUSUM. Les 3 facteurs sont
    multiplicatifs et chacun individuellement neutre (1.0) par défaut —
    aucun ne peut, à lui seul, faire disparaître une source."""
    base = compute_weights(vix, asset_type, urgency)
    all_srcs = list(base.keys())
    adjusted = {}
    for src, w_base in base.items():
        acc = _source_accuracy(asset_type, src, symbol, direction="", regime=regime)
        factor = 0.5 + acc
        indep = _pair_independence_factor(asset_type, src, all_srcs)
        drift_factor = 0.6 if is_drifting(asset_type, src) else 1.0
        w_new = w_base * factor * indep * drift_factor
        w_new = max(w_base * 0.7 * indep * drift_factor, min(w_base * 1.3, w_new))
        adjusted[src] = w_new
...
# ================================================================================
# [V28-ADD] GARDE-FOU WALK-FORWARD / OOS LÉGER
# Pas un vrai pipeline train/validation/OOS avec grid search (ça demanderait
# une infrastructure de backtesting séparée, hors de portée d'un ajout au
# scanner en ligne). Ceci est un garde-fou en continu : avant de faire
# confiance à une calibration, on vérifie qu'elle reste meilleure que la
# confiance brute sur la tranche la PLUS RÉCENTE de chaque bucket (20
# derniers échantillons vs les précédents) — si la calibration se dégrade
# sur la période récente, on arrête de l'utiliser jusqu'à ce qu'elle se
# stabilise à nouveau. C'est délibérément conservateur.
# ================================================================================
def calibration_still_valid(asset_type: str, raw_confidence: float) -> bool:
    label = _calib_bucket_label(max(0.0, min(0.999, raw_confidence)))
    if label is None:
        return False
    lst = _CALIB_STORE.get(asset_type, {}).get(label, [])
    if len(lst) < CALIB_MIN_PER_BUCKET + 20:
        return False  # pas assez pour comparer récent vs ancien -> prudence, refuse
    recent = lst[-20:]
    older = lst[:-20]
    recent_rate = sum(1 for x in recent if x) / len(recent)
    older_rate = sum(1 for x in older if x) / len(older) if older else recent_rate
    # La calibration reste valable si la tranche récente ne s'est pas
    # dégradée de plus de 15 points de pourcentage par rapport à l'ancienne.
    return (older_rate - recent_rate) <= 0.15


def calibrate_confidence_guarded(asset_type: str, raw_confidence: float) -> Dict:
    """Version de calibrate_confidence() qui applique en plus le garde-fou
    OOS ci-dessus — c'est CETTE fonction qu'il faut utiliser en pratique,
    pas calibrate_confidence() seule."""
    result = calibrate_confidence(asset_type, raw_confidence)
    if result["used_calibration"] and not calibration_still_valid(asset_type, raw_confidence):
        result["calibrated"] = raw_confidence
        result["used_calibration"] = False
        result["oos_guard_blocked"] = True
    return result
...
def compute_asset_state(
    ...
    # [V28-ADD] Régime de marché — calculé une fois, tôt, à partir des
    # données déjà en cache (aucun nouvel appel réseau), utilisé à la fois
    ...
    # [V139] Session = contexte de liquidité/volatilité, jamais veto directionnel
    session_sc, session_name, vol_mult, session_wr_buy = get_session_bias_score(sym, hour_utc)

    # [V28] record_pending_scores déplacé plus bas (après calcul de
    # `conviction`) pour pouvoir y attacher regime + confidence — voir juste
    # avant le `return AssetState(...)`. Les scores eux-mêmes restent
    # calculés ici, inchangés.
    _pending_scores_dict = {
    ...
    # [WS-NEW / MICRO-FIX] OFI impact sur score crypto
    # [MICRO-FIX] BUG CORRIGÉ : _ofi_v était déjà dans [-1,1], le *0.30 ici
    # ÉCRASAIT le signal une seconde fois en plus du poids appliqué dans
    # `final` (0.30 × 0.04 = poids réel max 1.2%). L'OFI (Cont/Kukanov/
    # Stoikov 2014) est le seul vrai signal order-flow L2 dispo côté crypto —
    # il garde maintenant son échelle native, le poids réel vient uniquement
    # de son coefficient dans `final` plus bas.
    _ofi_data  = _raw_cache.get("ofi", {}).get("data", {})
    _ofi_score = 0.0
    if typ == "crypto" and _ofi_data.get("ok"):
        _ofi_score = max(-1.0, min(1.0, float(_ofi_data.get("btc_ofi", 0.0))))
    ...
    # [MICRO-1] breath_composite — CVD(35%)+Funding(30%)+OI(15%)+Options(12%)+CME(8%)
    # était calculé et mis en cache à chaque cycle mais JAMAIS lu dans `final`
    # (seulement exposé en debug via breath_score dans le payload JSON). C'est
    # le pilier microstructure le plus complet déjà dans le code — il pilote
    # maintenant vraiment la direction, pas juste l'affichage.
    _breath_data  = _raw_cache.get("breath", {}).get("composite", {}).get("data", {})
    _breath_score = 0.0
    if typ == "crypto" and _breath_data.get("ok"):
        _breath_score = max(-1.0, min(1.0, float(_breath_data.get("breath_composite_score", 0.0))))

    # [MICRO-1] VPIN/BVC — Easley/López de Prado/O'Hara. Fonctionne sur TOUT
    # actif (BTC/XAU/XAG/forex) via OHLC+volume Yahoo déjà en cache —
    # contrairement à OFI/breath qui sont crypto-only (besoin L2 Binance/Coinbase).
    # C'est le vrai gain du Priorité-1 identifié plus tôt : Stage-1 sans
    # composante volume/flux nulle part ailleurs pour XAU/XAG/forex.
    _vpin_score = float(tech_data.get("vpin_score", 0.0)) if tech_data.get("ok") else 0.0

    # [MICRO-1] Wyckoff — était calculé, jamais appelé (code mort confirmé :
    # aucun appel à fetch_wyckoff_phases nulle part). Spring = retournement
    # haussier probable en fin d'accumulation ; Upthrust = symétrique baissier.
    _wyck_data  = _raw_cache.get("wyckoff", {}).get("data", {}).get(sym, {})
    _wyck_phase = _wyck_data.get("phase", "UNKNOWN")
    if _wyck_phase == "ACCUMULATION":
        _wyck_score = 0.10
    elif _wyck_phase == "DISTRIBUTION":
        _wyck_score = -0.10
    else:
        _wyck_score = 0.0
    if _wyck_data.get("spring"):
    ...
    # directionnelle, cf. docstring _calc_amihud). Appliquée plus bas comme
    # multiplicateur, sur le même modèle que _session_conf_mult/_contradiction_mult.
    _amihud_fragile = bool(tech_data.get("amihud_fragile", False)) if tech_data.get("ok") else False
    _amihud_conf_mult = 0.75 if _amihud_fragile else 1.0
    ...
    # [FIX-DIRECTION-BIAS] session_sc (biais directionnel fixe par session,
    # ex. +0.211 pour BTC pendant l'overlap Londres/NY) retiré du score
    # directionnel. Sources vérifiées (Semantic Scholar/CrossRef, DOIs
    # confirmés) : PLoS ONE 2017 (or/argent), Finance Research Letters
    # 2019/2020/2026 (BTC) démontrent une périodicité de VOLATILITÉ, VOLUME
    # et LIQUIDITÉ par session — AUCUNE ne démontre de biais DIRECTIONNEL fixe
    # par heure/session ("aucune étude sérieuse ne conclut que 08h=toujours
    # BUY"). vol_mult (lui, bien documenté) module maintenant la conviction
    # plus bas au lieu d'influencer la direction ici.
    final = round(max(-1.0, min(1.0,
        ...
        _ofi_score    * 0.06 +   # [MICRO-FIX] OFI — poids réel (bug ×0.30 corrigé)
        _liq_score    * 0.03 +   # [WS-NEW] Liquidations signal
        _breath_score * 0.09 +   # [MICRO-1] CVD+Funding+OI+Options+CME — rebranché (était mort)
        _vpin_score   * 0.07 +   # [MICRO-1] VPIN/BVC — order-flow toxicity, tout actif
        _wyck_score   * 0.05 +   # [MICRO-1] Wyckoff Accumulation/Distribution/Spring — rebranché
    ...
    # [FIX-CONTRADICTION-ENGINE] Les 9 sous-scores (news/etf/fg/macro/tech/
    # saisonnalité/OFI/liquidité/corrélation) sont sommés pondérés dans
    # `final`, mais rien ne vérifiait s'ils étaient d'accord entre eux. Un
    # score final proche de 0.10 peut cacher un vrai consensus faible OU une
    # bagarre entre signaux opposés qui s'annulent — deux situations très
    # différentes pour la fiabilité du trade. On mesure la part des sous-
    # scores significatifs (|score|>0.15) qui contredisent le signe de
    # `final`, et on réduit la conviction en proportion — jamais la direction
    # elle-même, cohérent avec le principe déjà appliqué à session_sc/vol_mult.
    _sub_scores = [news_sc, etf_sc, fg_sc, mac_sc, tech_sc, _seas_score, _ofi_score, _liq_score,
                   _breath_score, _vpin_score, _wyck_score]
    _significant = [s for s in _sub_scores if abs(s) > 0.15]
    _contradicting = [s for s in _significant if (s > 0) != (final > 0)]
    _contradiction_ratio = (len(_contradicting) / len(_significant)) if _significant else 0.0
    _contradiction_mult = max(0.55, 1.0 - _contradiction_ratio * 0.60)

    # [FIX-DIRECTION-BIAS] vol_mult = liquidité/volatilité de la session
    # active (ex. XAU overlap Londres/NY = 1.91) — proprement documenté par
    # les études vérifiées. Module la CONVICTION (signal plus fiable quand
    # le marché est liquide), pas la direction. Plage volontairement modeste
    # (0.80-1.25) pour rester un ajustement, pas un nouveau signal dominant.
    _session_conf_mult = max(0.80, min(1.25, 0.85 + (vol_mult - 1.0) * 0.15))
    # [MICRO-1] Amihud fragile = vide de liquidité qui se forme maintenant →
    # mouvement de prix pas confirmé par du volume réel, risque de reversal
    # violent. On amortit la conviction, jamais la direction (cf. docstring).
    conviction = round(min(1.0, abs(final) * 1.8 * _session_conf_mult
                           * _contradiction_mult * _amihud_conf_mult), 2)
    ...
    # [V139-SESSION] La session n'est JAMAIS un veto CameraOpen.
    # session_wr_buy reste une statistique descriptive/prior et peut alimenter
    # le sizing, mais ne peut plus transformer Asia/London/NY en NO_TRADE.
    # Seul le vrai contexte contrariant (RSI D1 extrême contre le sens demandé)
    # conserve ici un veto de sécurité directionnelle.
    session_cam_ok = True
    rsi_cam_ok     = True
    if contrarian_zone and direction != contrarian_dir:
        rsi_cam_ok = False

    camera_ok = rsi_cam_ok
    ...
    # [V28-ADD] Mémorise les scores de cycle pour évaluation différée (~1h),
    ...
    return AssetState(
        ...
        scores_detail     = {
            ...
            # [V28-ADD] Régime détecté + confiance calibrée (voir
        ...
        session_info = {
            ...
            # [V139] camera_allowed = sécurité directionnelle uniquement; session = contexte
...
async def fetch_twitter_impact(session: aiohttp.ClientSession) -> Dict:
    ...
    # ── VIP sentiment — Trump (Truth Social) ──────────────────────────────────
    # [FIX-RSS-DEAD → REMPLACÉ] Les 4 miroirs Nitter (nitter.net, nitter.poast.org)
    # sont morts (503/403, testé 19/07/2026). J'ai cherché des alternatives :
    # - trumpstruth.org/feed : RSS RÉEL et fonctionnel (testé, 100 items, contenu
    #   vérifié = vrais posts Trump récents). C'est même une meilleure source
    #   que l'ancien Nitter : Trump poste en priorité sur Truth Social depuis
    #   2021-2022, un miroir Twitter le voyait toujours en retard/incomplet.
    # - Elon Musk : AUCUNE alternative fiable et gratuite trouvée. Testé
    #   xcancel.com (nécessite whitelisting manuel par email, pas automatisable),
    #   nitter.tiekoetter.com (200 OK mais c'est en fait une page de challenge
    #   anti-bot "Anubis", pas du vrai contenu RSS), et plusieurs autres miroirs
    #   (tous 000/403/503). Le suivi Musk reste désactivé — vip_musk_detected
    #   restera toujours False tant qu'aucune source fiable n'est trouvée.
    ...
    try:
        async with session.get("https://trumpstruth.org/feed",
                               timeout=aiohttp.ClientTimeout(total=8),
                               headers=H_BROWSER) as r:
            if r.status == 200:
                text = await r.text()
                ...
                for item in items[1:6]:  # skip feed title
                    item_low = item.lower()
                    b = sum(1 for kw in trump_kw_bull if kw in item_low)
                    be = sum(1 for kw in trump_kw_bear if kw in item_low)
                    if b >= 1 or be >= 1:
                        vip_detected = True
                        vip_bull += b
                        vip_bear += be
...
def compute_btc_seasonality(hour_utc: int = -1) -> Dict:
    ...
    # [FIX-DIRECTION-BIAS-2] Poids horaire réduit 30%→15%. Sources vérifiées
    # le 19/07/2026 : Vojtko/Quantpedia (SSRN 10.2139/ssrn.4581124, "The
    # Seasonality of Bitcoin") est réel mais MAL ATTRIBUÉ ci-dessus — le
    # vrai co-auteur est Javorská, pas "Padysak" — et son contenu exact
    # (SSRN bloqué par captcha) n'a pas pu être confirmé pour le chiffre
    # H22-H23. AmberData 2025 : société réelle, mais article précis
    # introuvable. PocketOption : PAS une source de recherche — c'est un
    # broker d'options binaires (marketing, pas une preuve). Comparer à
    # SESSION_BIAS (retiré du score directionnel plus haut) : ici le biais
    # horaire est conservé mais fortement atténué plutôt que retiré, car
    # month_score/weekday_score reposent sur une littérature de saisonnalité
    # plus établie (effet janvier, etc. — non re-vérifiée spécifiquement
    # pour BTC ici, mais catégorie de recherche plus mature).
    # Mois : tendance de fond (55%, était 45%)
    # Jour  : biais structurel hebdo (30%, était 25%)
    # Heure : biais intraday (15%, était 30% — sourcing le plus faible)
    month_score   = (m_stat["wr"] - 0.50) * 2.0
    weekday_score = (w_stat["wr"] - 0.50) * 2.0
    hourly_score  = h_stat["bias"]  # déjà en -1.0 à +1.0

    score = month_score * 0.55 + weekday_score * 0.30 + hourly_score * 0.15
...
class WorldSentinel:
    def __init__(self):
        ...
        self._last_real_yield = 0.0
        self._last_cot = 0.0
        self._last_yield_curve = 0.0
        self._last_imf_gold = 0.0
        self._last_comex_stocks = 0.0
        ...
        self._urgency       = False
        self._cycle_count   = 0
        # [FIX-V26-HEALTH-VISIBILITY] 5 sources appelaient self._track() avec un
        # nom absent de ce dict ("fred_real_yield","cot_gold","yield_curve",
        # "comex_stocks","imf_gold" — voir les appels plus bas) — _track() les
        # ignore silencieusement (if source in self._source_health: ...), donc
        # même en panne totale et permanente, ces 5 sources n'apparaissaient
        # JAMAIS dans le log [HEALTH]. Ce sont précisément les 5 sources
        # fondamentales or/macro les plus récentes (real_yield_10y, COT gold,
        # yield curve, stocks COMEX, réserves or FMI) — celles qui alimentent
        # cot_gold_net_pct / real_yield_10y / yield_curve_10y2y /
        # cb_gold_reserves_trend / comex_registered_change_pct envoyés à
        # staline_server. Aucune logique changée : on ajoute juste ces 5 clés
        # pour qu'elles deviennent visibles dans le log [HEALTH] comme les 9
        # autres — c'est le diagnostic exact qu'il faut pour savoir si elles
        # tournent vraiment ou si elles retombent en fallback en silence.
        self._source_health = {k: {"ok": 0, "fail": 0} for k in
                               ["yahoo", "binance", "coingecko", "rss", "fg",
                                "fred", "fred_cpi", "fred_jolts", "fred_nfp",
                                "fred_real_yield", "cot_gold", "yield_curve",
                                "comex_stocks", "imf_gold"]}
        ...
        while self._running:
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                    ...
                    # ── [NOUVEAU] FRED taux réel 10 ans (TIPS, DFII10) ────────
                    # Facteur fondamental établi pour l'or, absent jusqu'ici.
                    if self._is_stale(self._last_real_yield, fred_ttl):
                        real_yield = await fetch_fred_real_yield(session)
                        ok = real_yield.get("ok", False)
                        self._track("fred_real_yield", ok)
                        _raw_cache["fred_real_yield"] = {"data": real_yield, "ts": time.time()}
                        self._last_real_yield = time.time()
                        if ok:
                            updated.append(f"REAL_YIELD={real_yield.get('real_yield_10y',0):.2f}%")

                    # ── [NOUVEAU] COT réel (CFTC, hebdomadaire) — remplace le
                    # proxy figé côté serveur.
                    if self._is_stale(self._last_cot, 3600 * 12):  # re-check 2x/jour, donnée hebdo de toute façon
                        cot_gold = await fetch_cot_report(session, "088691")
                        ok = cot_gold.get("ok", False)
                        self._track("cot_gold", ok)
                        _raw_cache["cot_gold"] = {"data": cot_gold, "ts": time.time()}
                        self._last_cot = time.time()
                        if ok:
                            updated.append(f"COT_GOLD={cot_gold.get('net_position_pct',0):.2f}")

                    # ── [NOUVEAU] Courbe des taux 10Y-2Y (indicateur récession) ──
                    if self._is_stale(self._last_yield_curve, fred_ttl):
                        yc = await fetch_yield_curve(session)
                        ok = yc.get("ok", False)
                        self._track("yield_curve", ok)
                        _raw_cache["yield_curve"] = {"data": yc, "ts": time.time()}
                        self._last_yield_curve = time.time()
                        if ok:
                            updated.append(f"YIELD_CURVE={yc.get('yield_curve_10y2y',0):.2f}")

                    # ── [NOUVEAU-COMPLET] Stocks physiques COMEX (quotidien) ──
                    if self._is_stale(self._last_comex_stocks, 3600 * 20):
                        comex = await fetch_comex_warehouse_stocks(session)
                        ok = comex.get("ok", False)
                        self._track("comex_stocks", ok)
                        _raw_cache["comex_stocks"] = {"data": comex, "ts": time.time()}
                        self._last_comex_stocks = time.time()
                        if ok:
                            updated.append(f"COMEX_REG_CHG={comex.get('registered_change_pct',0):.2f}%")

                    # ── [NOUVEAU] Réserves d'or banques centrales (FMI, mensuel) ──
                    if self._is_stale(self._last_imf_gold, 3600 * 24):  # 1x/jour, donnée mensuelle
                        imf_gold = await fetch_imf_gold_reserves(session)
                        ok = imf_gold.get("ok", False)
                        self._track("imf_gold", ok)
                        _raw_cache["imf_gold"] = {"data": imf_gold, "ts": time.time()}
                        self._last_imf_gold = time.time()
                        if ok:
                            updated.append(f"IMF_GOLD={imf_gold.get('reserves_trend','?')}")
                    ...
                    if tech_tasks:
                        ...
                        # ── [MICRO-2] Wyckoff phases — était calculé nulle part,
                        # code mort. Réutilise les candles 1d déjà en cache.
                        TTL_WYCKOFF_LOOP = 600
                        if self._is_stale(self._last_wyckoff, TTL_WYCKOFF_LOOP):
                            _symbols_ohlc = {
                                s: r.get("ohlc_1d", [])
                                for s, r in _raw_cache.get("technicals", {}).items()
                                if isinstance(r, dict) and r.get("ohlc_1d")
                            }
                            if _symbols_ohlc:
                                try:
                                    _wyck = await fetch_wyckoff_phases(session, _symbols_ohlc)
                                    _raw_cache["wyckoff"] = {"data": _wyck, "ts": time.time()}
                                    self._last_wyckoff = time.time()
                                    if _wyck:
                                        updated.append(f"WYCKOFF×{len(_wyck)}")
                                except Exception as _wyck_e:
                                    logger.debug("[WYCKOFF] %s", _wyck_e)
                    ...
                    _geo_tasks_needed = (
                        ...
                        self._is_stale(self._last_twitter, TTL_TWITTER)
                    )
    ...
    async def _push_to_server(self):
        ...
        with _shm_lock:
            ...
            for sym, st in SHM_STATE.items():
                ...
                results[sym] = {
                    ...
                    # [V28-ADD] Régime détecté + calibration + alertes de drift —
                    # champs additionnels au niveau racine pour un accès direct
                    # côté serveur (comme les autres champs racine ci-dessous),
                    # rien d'existant n'est retiré ni modifié.
                    "regime":              sc.get("regime", "UNKNOWN"),
                    "calibration":         sc.get("calibration", {}),
                    "drift_alert_sources": sc.get("drift_alert_sources", []),
                    ...
                    # [NOUVEAU] Taux réel 10 ans (TIPS) — facteur fondamental établi
                    # pour l'or, absent du scanner jusqu'ici.
                    "real_yield_10y":     _raw_cache.get("fred_real_yield", {}).get("data", {}).get("real_yield_10y", 1.80),
                    # [NOUVEAU] Vraie donnée COT or (CFTC) — remplace le proxy figé
                    "cot_gold_net_pct":   _raw_cache.get("cot_gold", {}).get("data", {}).get("net_position_pct", None),
                    # [NOUVEAU] Courbe des taux 10Y-2Y — indicateur de récession établi
                    "yield_curve_10y2y":  _raw_cache.get("yield_curve", {}).get("data", {}).get("yield_curve_10y2y", 0.50),
                    # [NOUVEAU] Saisonnalité calendaire de l'or (demande physique Inde/Chine)
                    "gold_seasonal_bias": compute_gold_seasonality().get("gold_seasonal_bias", 0.0),
                    # [NOUVEAU] Ratio Or/Argent — calculé depuis les prix déjà collectés
                    # (fetch_macro_yahoo), zéro nouvelle donnée externe nécessaire.
                    "gold_silver_ratio":  (lambda _g=_raw_cache.get("macro", {}).get("data", {}).get("gold", 0.0),
                                                  _s=_raw_cache.get("macro", {}).get("data", {}).get("silver", 0.0):
                                            round(_g/_s, 2) if _g and _s else None)(),
                    # [NOUVEAU] Tendance réserves d'or banques centrales (FMI, mensuel)
                    "cb_gold_reserves_trend": _raw_cache.get("imf_gold", {}).get("data", {}).get("reserves_trend", "UNKNOWN"),
                    # [NOUVEAU-COMPLET] Variation stocks physiques COMEX (parsing réel)
                    "comex_registered_change_pct": _raw_cache.get("comex_stocks", {}).get("data", {}).get("registered_change_pct", 0.0),
                    ...
                    "source":            "WORLD_SCANNER_v3.9_SESSION_CONTEXT",
        ...
        payload = {
            "results":   results,
            "scanner":   "WORLD_SCANNER_v3.9_SESSION_CONTEXT",
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "urgency":   self._urgency,
        }
