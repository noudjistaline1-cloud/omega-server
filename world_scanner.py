...
# ================================================================================
# ANALYSE TECHNIQUE — Calcul RSI + SMA sur données Yahoo Finance réelles
# Sources : Yahoo Finance v8 5m (confirmé sur Render)
# Calcule : RSI-14, SMA-20, SMA-50, momentum, signal technique
# Pas de scraping — calcul pur sur données de prix
# ================================================================================
...
def _calc_rsi(closes: list, period: int = 14) -> float:
    """RSI-14 sur liste de prix de clôture."""
...
def _interpret_technicals(rsi: float, price: float, sma20: float, sma50: float,
                           momentum_pct: float, typ: str) -> dict:
    """
    Interprète les indicateurs techniques.
    Retourne signal et score (-1.0 à +1.0).
    """
    score = 0.0
    signals = []
    ...
    if rsi >= 70:
        score -= 0.30
        signals.append(f"RSI={rsi:.0f} SURACHETÉ")
    elif rsi >= 60:
        score += 0.15
        signals.append(f"RSI={rsi:.0f} fort")
    elif rsi <= 30:
        score += 0.30
        signals.append(f"RSI={rsi:.0f} SURVENDU")
    elif rsi <= 40:
        score -= 0.15
        signals.append(f"RSI={rsi:.0f} faible")
    else:
        signals.append(f"RSI={rsi:.0f} neutre")

    # Prix vs SMA (tendance)
...
async def fetch_technicals_for_symbol(session: aiohttp.ClientSession,
                                       sym: str, profile: Dict) -> Dict:
    """
    Calcule RSI-14 + SMA-20/50 + momentum sur données Yahoo 5m.
    Source : Yahoo Finance v8 — confirmé sur Render Frankfurt.
    """
    result = {"signal":"NEUTRAL","score":0.0,"rsi":50.0,"ok":False}
    yahoo_sym = YAHOO_SYMBOL_MAP.get(sym)
    if not yahoo_sym:
        return result

    # Télécharger 2 jours en 5m = ~78 bougies par jour = suffisant pour RSI-14 + SMA-50
    for url_tpl in [
        f"https://query1.finance.yahoo.com/v8/finance/chart/{yahoo_sym}?range=2d&interval=5m",
        f"https://query2.finance.yahoo.com/v8/finance/chart/{yahoo_sym}?range=2d&interval=5m",
    ]:
        for headers in [H_BROWSER, H_BOT]:
            try:
                async with session.get(url_tpl, timeout=aiohttp.ClientTimeout(total=9),
                    ...
                    if r.status == 200:
                        ...
                        open10      = closes[-10] if len(closes) >= 10 else closes[0]
                        momentum_pct = ((price - open10) / open10 * 100) if open10 > 0 else 0.0

                        tech = _interpret_technicals(
                            rsi, price, sma20, sma50, momentum_pct,
                            profile.get("type","forex")
                        )
                        tech["ok"]    = True
...
async def fetch_forexfactory_events(session: aiohttp.ClientSession) -> List[Dict]:
    """
    Calendrier économique ForexFactory — événements haute impact aujourd'hui.
    URL : https://nfs.faireconomy.media/ff_calendar_thisweek.json
    Gratuit, sans clé, JSON direct.
    Retourne les événements HIGH impact pour les prochaines 24h.
    """
    result = []
    try:
            ...
            if r.status == 200:
                ...
                for ev in events:
                    try:
                        ...
                        # Parse ISO ou format FF
                        for fmt in ["%Y-%m-%dT%H:%M:%S%z", "%Y-%m-%dT%H:%M:%S"]:
                            try:
                                ev_time = datetime.strptime(ev_time_str[:19], fmt[:fmt.index('%z') if '%z' in fmt else len(fmt)])
...
async def fetch_macro_yahoo(session: aiohttp.ClientSession) -> Dict:
    ...
    tickers_v8 = {
        ...
        "nasdaq": "^IXIC",
        "gold":   "GC=F",
        "silver": "SI=F",      # XAGUSD — ajouté pour prix réel
        "btc_yf": "BTC-USD",   # BTC via Yahoo (fallback Binance)
            ...
            for headers in [H_BROWSER, H_BOT]:
                try:
                    if api_ver == "v8":
                        ...
                        url = f"https://query2.finance.yahoo.com/v8/finance/chart/{ticker}?range=1d&interval=5m"
...
def compute_weights(vix: float, urgency: bool = False) -> Dict[str, float]:
    ...
    5 sources : news + etf + fg + macro + technique
    Urgence/crise → ETF et news dominent (flux réels)
    Calme → technique et sentiment reprennent de l'importance
    """
    if urgency or vix > 30:
        # CRISE MAJEURE : flux réels et géopolitique dominent, technique = bruit
        return {"news":0.38, "etf":0.35, "fg":0.05, "macro":0.12, "tech":0.10}
    elif vix > VIX_STRESS:
        # STRESS MODÉRÉ : ETF + news restent dominants
        return {"news":0.33, "etf":0.30, "fg":0.07, "macro":0.15, "tech":0.15}
    else:
        # MARCHÉ CALME : technique et sentiment reprennent de l'importance
        return {"news":0.25, "etf":0.23, "fg":0.18, "macro":0.17, "tech":0.17}
...
def score_macro(macro: Dict, binance: Dict, cg: Dict, fred: Dict, profile: Dict) -> float:
    ...
    if typ == "metal":
        s = 0.0
        # DXY : or/argent inverse du dollar
        if dxy > 105: s -= 0.50
        elif dxy > 103: s -= 0.25
        elif dxy < 98: s += 0.50
        elif dxy < 100: s += 0.25
        # VIX : métaux = valeur refuge en stress
        if vix > 30: s += 0.45
        elif vix > 25: s += 0.25
        elif vix > 22: s += 0.12
        elif vix < 12: s -= 0.15
        # Taux : hausse = bearish or/argent
        if u10 > 5.0: s -= 0.40
        elif u10 > 4.7: s -= 0.20
        elif u10 < 3.5: s += 0.30
        elif u10 < 4.0: s += 0.12
        ...
        # Silver spécifique : momentum prix vs Gold
        ...
        if silver_price > 0 and gold_price > 0:
            ...
            # Ratio or/argent > 85 → argent sous-évalué → bullish XAG
            if gold_silver_ratio > 90: s += 0.15
            elif gold_silver_ratio > 85: s += 0.08
            elif gold_silver_ratio < 70: s -= 0.10
        # SP500 corrélé argent (plus industriel que l'or)
        if "XAG" in profile.get("sym",""):
            if spx > 5400: s += 0.08   # Argent = métal industriel
...
def compute_asset_state(
    ...
    # Score technique (RSI + SMA + momentum) — calculé en parallèle par le sentinel
    tech_data = _raw_cache.get("technicals", {}).get(sym, {})
    tech_sc   = float(tech_data.get("score", 0.0)) if tech_data.get("ok") else 0.0

    # Événements ForexFactory haute impact → réduire score si évènement imminent
    ff_events  = _raw_cache.get("ff_events", {}).get("data", [])
    ff_penalty = 0.0
    for ev in ff_events:
        if ev.get("impact") == "HIGH":
            # Événement HIGH impact dans 24h → on réduit l'exposition de 20%
            ff_penalty = -0.15
            break
        elif ev.get("impact") == "MEDIUM":
            ff_penalty = -0.07

    # Score final pondéré — 5 sources
    final = round(max(-1.0, min(1.0,
        ...
        mac_sc  * w["macro"] +
        tech_sc * w["tech"]  +
        ff_penalty           # Réduction si évènement macro imminent
    )), 4)
    ...
    # Signal technique RSI/SMA
    if tech_data.get("ok"):
        rsi_v = tech_data.get("rsi", 50)
        tech_sigs = " | ".join(tech_data.get("signals", [])[:2])
        reasons.append(f"[TECH] {tech_data.get('signal','?')} RSI={rsi_v:.0f} | {tech_sigs}")
    # ForexFactory événements
    if ff_events:
        high_ev = [e for e in ff_events if e.get("impact")=="HIGH"]
        if high_ev:
            ev0 = high_ev[0]
            reasons.append(f"[FF_HIGH] {ev0.get('currency','?')} — {ev0.get('title','?')[:50]} @ {ev0.get('time','')[:16]}")
    if cg.get("ok"):
        ...
        while self._running:
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout,
                    ...
                    # ── TECHNIQUE RSI/SMA + ForexFactory ─────────────────────
                    TTL_TECH = 180
                    if not hasattr(self, "_last_tech"): self._last_tech = {}
                    tech_tasks_2 = {}
                    for s2, p2 in ASSET_PROFILES.items():
                        if s2 in YAHOO_SYMBOL_MAP and self._is_stale(self._last_tech.get(s2,0.0), TTL_TECH):
                            tech_tasks_2[s2] = fetch_technicals_for_symbol(session, s2, p2)
                    if tech_tasks_2:
                        tech_res_2 = await asyncio.gather(*tech_tasks_2.values(), return_exceptions=True)
                        if "technicals" not in _raw_cache: _raw_cache["technicals"] = {}
                        t_ok = 0
                        for s2, r2 in zip(tech_tasks_2.keys(), tech_res_2):
                            if isinstance(r2, dict):
                                _raw_cache["technicals"][s2] = r2
                                self._last_tech[s2] = time.time()
                                if r2.get("ok"): t_ok += 1
                        if t_ok: updated.append(f"TECH×{t_ok}")
                    TTL_FF = 1800
                    if not hasattr(self, "_last_ff"): self._last_ff = 0.0
                    if self._is_stale(self._last_ff, TTL_FF):
                        ff_ev2 = await fetch_forexfactory_events(session)
                        _raw_cache["ff_events"] = {"data": ff_ev2, "ts": time.time()}
                        self._last_ff = time.time()
                        h2 = [e for e in ff_ev2 if e.get("impact")=="HIGH"]
                        if ff_ev2: updated.append(f"FF(H×{len(h2)})")
