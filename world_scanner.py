...
_raw_cache = {
    ...
    "newsapi":    {"data": {}, "ts": 0.0},   # [BOOST] News sentiment
    "reddit":     {"data": {}, "ts": 0.0},   # [BOOST] Reddit sentiment
    "econocal":   {"data": {}, "ts": 0.0},   # [BOOST] Economic calendar
    "fomc":       {"data": {}, "ts": 0.0},
    "pmi":        {"data": {}, "ts": 0.0},
    "twitter":    {"data": {}, "ts": 0.0},
    "session":    {"data": {}, "ts": 0.0},
...
TTL_ONCHAIN  = 300    # On-chain metrics (Glassnode free / blockchain.info) — 5 min
TTL_NEWSAPI  = 180    # NewsAPI.org — 3 min (500 req/jour free tier)
TTL_REDDIT   = 240    # Reddit WallStreetBets + CryptoMoonShots — 4 min
TTL_ECONOCAL = 1800   # Economic calendar (ForexFactory RSS) — 30 min (données lentes)
...
# ── [V16] INTRADAY CROSS MATRIX BTC — Jour × Tranche horaire ─────────────────
# Alignée avec staline_server_V16.py compute_intraday_cross_bias
# Source: Baur&Cahill 2022 JBF + Aharon&Qadan 2018 + QuantifiedStrategies 2023
_BTC_CROSS = {
    0: {"0000_0500":("SELL",0.58,-0.45,0.82),"0500_1100":("BUY_SQUEEZE",0.54,0.65,1.05),
        "1100_1700":("SELL",0.61,-0.85,1.30),"1700_2130":("NEUTRAL",0.50,0.05,1.00),"2130_2400":("BUY",0.55,0.40,0.90)},
    1: {"0000_0500":("NEUTRAL",0.51,-0.05,0.85),"0500_1100":("BUY",0.56,0.70,1.08),
        "1100_1700":("BUY",0.59,0.95,1.38),"1700_2130":("SELL",0.54,-0.35,1.15),"2130_2400":("SELL",0.57,-0.50,0.88)},
    2: {"0000_0500":("SELL",0.53,-0.30,0.80),"0500_1100":("NEUTRAL",0.49,0.02,0.92),
        "1100_1700":("VOLATILE",0.62,1.40,1.55),"1700_2130":("BUY",0.55,0.50,1.20),"2130_2400":("BUY",0.58,0.65,1.00)},
    3: {"0000_0500":("BUY",0.53,0.25,0.83),"0500_1100":("SELL",0.56,-0.45,1.02),
        "1100_1700":("SELL",0.58,-0.80,1.28),"1700_2130":("NEUTRAL",0.50,-0.05,0.95),"2130_2400":("BUY",0.60,0.75,0.90)},
    4: {"0000_0500":("BUY",0.55,0.40,0.90),"0500_1100":("BUY",0.57,0.60,1.10),
        "1100_1700":("BUY",0.63,1.10,1.40),"1700_2130":("SELL",0.59,-0.70,1.25),"2130_2400":("NEUTRAL",0.51,0.10,0.88)},
    5: {"0000_0800":("NEUTRAL",0.52,0.05,0.65),"0800_1800":("BUY",0.56,0.25,0.72),"1800_2400":("BUY",0.57,0.40,0.75)},
    6: {"0000_0800":("NEUTRAL",0.52,0.05,0.68),"0800_1800":("BUY",0.54,0.20,0.78),
        "1800_2000":("VOLATILE",0.52,0.90,1.12),"2000_2400":("SELL",0.55,-0.35,0.92)},
}

def _cross_slot(hour_utc: int, minute_utc: int, dow: int) -> str:
    t = hour_utc * 100 + minute_utc
    if dow in (5, 6):
        if t < 800: return "0000_0800"
        if t < 1800: return "0800_1800"
        return ("1800_2000" if t < 2000 else "2000_2400") if dow == 6 else "1800_2400"
    if t < 500: return "0000_0500"
    if t < 1100: return "0500_1100"
    if t < 1700: return "1100_1700"
    if t < 2130: return "1700_2130"
    return "2130_2400"

def get_btc_intraday_cross(hour_utc: int, minute_utc: int = 0) -> Dict:
    """[V16] Biais croisé Jour×Heure pour BTC — utilisé dans compute_btc_seasonality."""
    from datetime import datetime, timezone
    dow = datetime.now(timezone.utc).weekday()
    slot = _cross_slot(hour_utc, minute_utc, dow)
    entry = _BTC_CROSS.get(dow, {}).get(slot, ("NEUTRAL", 0.50, 0.00, 1.00))
    bias, wr, avg_move, vol_rel = entry
    # Modifier score: BUY=+0.12, SELL=-0.12, SQUEEZE=+0.06, VOLATILE=0, NEUTRAL=0
    mods = {"BUY":0.12,"BUY_SQUEEZE":0.06,"BUY_DRIFT":0.08,"SELL":-0.12,"SELL_DRIFT":-0.08}
    mod = mods.get(bias, 0.0)
    return {
        "bias": bias, "win_rate": wr, "avg_move_pct": avg_move,
        "vol_rel": vol_rel, "slot": slot, "dow": dow,
        "score_modifier": round(mod, 4),
        "is_squeeze_trap": bias == "BUY_SQUEEZE",
        "is_volatile": bias == "VOLATILE",
    }
...
def compute_btc_seasonality(hour_utc: int = -1) -> Dict:
    ...
    _min_utc = _dt2.now(_tz2.utc).minute
    ...
    # [V16] Biais croisé Jour×Heure (plus précis que mois/semaine/heure séparés)
    cross = get_btc_intraday_cross(hour_utc, _min_utc)

    # ── Score composite pondéré V16 ───────────────────────────────────────
    # V16: mois 35% + jour 15% + heure 25% + cross 25% (croisement prime)
    month_score   = (m_stat["wr"] - 0.50) * 2.0
    weekday_score = (w_stat["wr"] - 0.50) * 2.0
    hourly_score  = h_stat["bias"]
    cross_score   = cross["score_modifier"] / 0.12  # normalise [-1, +1]

    score = month_score * 0.35 + weekday_score * 0.15 + hourly_score * 0.25 + cross_score * 0.25
    ...
    return {
        ...
        "signal":               "SEASONAL_BULL"    if final_score > 0.15  else
                                ...
                                "SEASONAL_NEUTRAL",
        # [V16] Champs intraday cross matrix
        "intraday_bias":        cross["bias"],
        "intraday_slot":        cross["slot"],
        "intraday_wr":          cross["win_rate"],
        "intraday_vol_rel":     cross["vol_rel"],
        "is_squeeze_trap":      cross["is_squeeze_trap"],
        "is_volatile_slot":     cross["is_volatile"],
        "intraday_score_mod":   cross["score_modifier"],
    }
        ...
        with _shm_lock:
            ...
            for sym, st in SHM_STATE.items():
                ...
                results[sym] = {
                    ...
                    # [V16] Intraday cross matrix — branché sur serveur V16
                    "intraday_bias":     _raw_cache.get("seasonality",{}).get("data",{}).get("intraday_bias","NEUTRAL"),
                    "intraday_slot":     _raw_cache.get("seasonality",{}).get("data",{}).get("intraday_slot",""),
                    "intraday_vol_rel":  _raw_cache.get("seasonality",{}).get("data",{}).get("intraday_vol_rel",1.0),
                    "is_squeeze_trap":   _raw_cache.get("seasonality",{}).get("data",{}).get("is_squeeze_trap",False),
                    "is_volatile_slot":  _raw_cache.get("seasonality",{}).get("data",{}).get("is_volatile_slot",False),
                    ...
                    "stale":             False,
                    # [BOOST] 4 nouvelles sources
                    "onchain":           _raw_cache.get("onchain",  {}).get("data", {}),
                    ...
                    "news_top":          _raw_cache.get("newsapi",  {}).get("data", {}).get("top_titles",      []),
