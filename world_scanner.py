# ================================================================================
# world_scanner_v3.py — OMEGA FUSION BRAIN v3.0 — EDITION INGENIEUR EA TRADING
# ================================================================================
#
...
# LANCER  : python world_scanner_v3.py
# ================================================================================
...
logger = logging.getLogger("world_scanner_v3")
...
SESSION_BIAS = {
    # "actif": {"SESSION": {"bias": +1=bull/-1=bear, "vol_mult": facteur de volatilité, "wr_buy": winrate BUY historique}}
    "XAUUSD": {
        "TOKYO":    {"bias": 0.0,  "vol_mult": 0.7, "wr_buy": 0.50},
        "LONDON":   {"bias": +0.3, "vol_mult": 1.3, "wr_buy": 0.58},  # London gold fix 10h30
        "NEW_YORK": {"bias": -0.1, "vol_mult": 1.5, "wr_buy": 0.52},  # USD dominant après Comex open
        "OVERLAP_LN_NY": {"bias": +0.2, "vol_mult": 1.8, "wr_buy": 0.61},
    },
    "XAGUSD": {
        "TOKYO":    {"bias": 0.0,  "vol_mult": 0.6, "wr_buy": 0.50},
        "LONDON":   {"bias": +0.2, "vol_mult": 1.2, "wr_buy": 0.55},
        "NEW_YORK": {"bias": -0.2, "vol_mult": 1.6, "wr_buy": 0.48},
    },
    "EURUSD": {
        "TOKYO":    {"bias": 0.0,  "vol_mult": 0.5, "wr_buy": 0.50},
        "LONDON":   {"bias": +0.1, "vol_mult": 1.4, "wr_buy": 0.53},
        "NEW_YORK": {"bias": -0.1, "vol_mult": 1.3, "wr_buy": 0.49},
        "OVERLAP_LN_NY": {"bias": 0.0, "vol_mult": 1.7, "wr_buy": 0.51},
    },
    "GBPUSD": {
        "TOKYO":    {"bias": 0.0,  "vol_mult": 0.4, "wr_buy": 0.50},
        "LONDON":   {"bias": +0.2, "vol_mult": 1.5, "wr_buy": 0.55},
        "NEW_YORK": {"bias": -0.1, "vol_mult": 1.2, "wr_buy": 0.49},
    },
    "USDJPY": {
        "TOKYO":    {"bias": +0.2, "vol_mult": 1.4, "wr_buy": 0.57},  # JPY actif en session Tokyo
        "LONDON":   {"bias": 0.0,  "vol_mult": 0.9, "wr_buy": 0.51},
        "NEW_YORK": {"bias": +0.1, "vol_mult": 1.1, "wr_buy": 0.53},
    },
    "GBPJPY": {
        "TOKYO":    {"bias": 0.0,  "vol_mult": 1.2, "wr_buy": 0.51},
        "LONDON":   {"bias": +0.3, "vol_mult": 1.8, "wr_buy": 0.59},  # GBP + JPY croisement explosif
        "NEW_YORK": {"bias": 0.0,  "vol_mult": 1.0, "wr_buy": 0.50},
        "OVERLAP_LN_NY": {"bias": +0.2, "vol_mult": 2.0, "wr_buy": 0.58},
    },
    "BTCUSD": {
        "TOKYO":    {"bias": 0.0,  "vol_mult": 0.8, "wr_buy": 0.50},
        "LONDON":   {"bias": +0.1, "vol_mult": 1.1, "wr_buy": 0.52},
        "NEW_YORK": {"bias": +0.2, "vol_mult": 1.4, "wr_buy": 0.55},  # US institutional hours
        "OVERLAP_LN_NY": {"bias": +0.1, "vol_mult": 1.3, "wr_buy": 0.53},
    },
    "ETHUSD": {
        "TOKYO":    {"bias": 0.0,  "vol_mult": 0.8, "wr_buy": 0.50},
        "LONDON":   {"bias": +0.1, "vol_mult": 1.1, "wr_buy": 0.52},
        "NEW_YORK": {"bias": +0.2, "vol_mult": 1.5, "wr_buy": 0.55},
    },
    "US30":  {
        "LONDON":   {"bias": -0.1, "vol_mult": 0.8, "wr_buy": 0.49},  # Pre-market nerveux
        "NEW_YORK": {"bias": +0.2, "vol_mult": 1.6, "wr_buy": 0.56},  # Cash session 14h30
    },
    "US100": {
        "LONDON":   {"bias": -0.1, "vol_mult": 0.7, "wr_buy": 0.49},
        "NEW_YORK": {"bias": +0.2, "vol_mult": 1.7, "wr_buy": 0.57},
    },
    "US500": {
        "LONDON":   {"bias": -0.1, "vol_mult": 0.7, "wr_buy": 0.49},
        "NEW_YORK": {"bias": +0.2, "vol_mult": 1.6, "wr_buy": 0.56},
    },
}
...
def get_session_bias_score(sym: str, hour_utc: int) -> Tuple[float, str, float]:
    """
    Calcule le score directionnel de la session active pour un actif.
    Retourne (score: -1.0 à +1.0, session_name, vol_multiplier).

    Exemple : XAUUSD à 14h UTC → OVERLAP_LN_NY → bias=+0.2, vol=1.8
    """
...
def get_trade_mode(
    ...
    session_score, session_name, vol_mult = get_session_bias_score(sym, hour_utc)
...
ASSET_PROFILES = {
    # geo_pol est maintenant DYNAMIQUE — valeur initiale conservatrice
    # Mis à jour en temps réel par _compute_dynamic_geo_pol()
...
def compute_asset_state(
    ...
    # [FIX-3] Score session horaire
    session_sc, session_name, vol_mult = get_session_bias_score(sym, hour_utc)
    ...
    reasons.append(f"[SESSION] {session_name} | vol×{vol_mult:.1f} | bias={session_sc:+.2f}")
...
class WorldSentinel:
    ...
    async def run(self):
        logger.info("=" * 70)
        logger.info("  WORLD SENTINEL v3.1 — OMEGA FUSION BRAIN — EDITION EA TRADING")
    ...
    async def _push_to_server(self):
        """[FIX-10] Push enrichi avec session_info et scorecard dans le payload."""
        with _shm_lock:
            results = {}
