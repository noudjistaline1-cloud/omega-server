...
def _source_accuracy(asset_type: str, source: str, symbol: str = "") -> float:
    """Accuracy glissante avec shrinkage Beta + pondération exponentielle récente.
    Préfère le profil symbole, puis retombe sur le profil asset global."""
    keys = []
    if symbol:
    ...
    n=len(lst)
    if n < 5: return 0.5
    # poids exponentiels: les observations récentes dominent, sans oublier l'historique.
    decay=0.965
    weights=[decay**(n-1-i) for i in range(n)]
    den=sum(weights) or 1.0
    raw=sum(w*(1.0 if ok else 0.0) for w,ok in zip(weights,lst))/den
    # prior Beta(5,5): empêche un petit échantillon de créer un faux edge.
    prior_n=min(20,n)
    shrunk=(raw*n + 0.5*prior_n)/(n+prior_n)
    return max(0.20,min(0.80,shrunk))


def compute_weights_adaptive(vix: float, asset_type: str = "forex", urgency: bool = False, symbol: str = "") -> Dict[str, float]:
    ...
    for src, w_base in base.items():
        acc = _source_accuracy(asset_type, src, symbol)
...
def compute_asset_state(
    ...
    w = compute_weights_adaptive(vix, typ, urgency, symbol=sym)
