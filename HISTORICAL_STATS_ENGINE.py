    ...
    if hourly_bars:
        ...
        for h in range(24):
            ...
            if len(rets) >= 20:
                ...
                # [FIX-ASYMÉTRIE] L'ancienne formule ne pondérait QUE les bougies
                # haussières fortes (×2 au numérateur), sans jamais pondérer les
                # bougies baissières fortes de façon symétrique — et le
                # dénominateur ("total_strong + (len(rets)-total_strong)") se
                # simplifiait mathématiquement en simple len(rets), annulant tout
                # effet de pondération au dénominateur. Résultat concret : un
                # échantillon parfaitement équilibré (10 bull/10 bear, 5 bull forts,
                # 5 bear forts) donnait weighted_bull=0.75 au lieu de 0.50 — un
                # biais haussier injecté par construction, sans rapport avec les
                # vraies données. Corrigé : les bougies fortes (bull ET bear)
                # pèsent 2x, appliqué de façon cohérente au numérateur ET au
                # dénominateur.
                ...
                bull_weight = strong_bull * 2 + (bull - strong_bull) * 1
                bear_weight = strong_bear * 2 + (bear - strong_bear) * 1
                neutral_n   = len(rets) - bull - bear  # candles à variation nulle exacte
                total_weight = bull_weight + bear_weight + neutral_n
                weighted_bull = (bull_weight / total_weight) if total_weight > 0 else 0.5
...
def main():
    ...
    output = {
        "generated_at": datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"),  # Format ISO
        "version":       f"HSE-v{datetime.datetime.utcnow().strftime('%Y%m%d')}",
...
# ============================================================================
# MODULE INTERFACE — Fonctions attendues par staline_server_V14.py
# Le serveur importe ces 4 fonctions via:
#   from HISTORICAL_STATS_ENGINE import (
#       get_market_hourly_bias, is_transition_danger_zone,
#       init_historical_stats, get_status
#   )
# ============================================================================
...
# Heures de transition (direction change imminente) — par symbole
# Basé sur analyse des 33 012 trades réels Jan-Avr 2026
_TRANSITION_HOURS = {
    "XAUUSD": {13, 17, 18},  # H13→H14 SELL→BUY, H17→H18 SELL→BUY
    "XAGUSD": {9, 17, 18},
    "EURUSD": {1, 11, 22},
    "USDJPY": {6, 11, 19},
    "BTCUSD": {1, 9, 13},
...
def init_historical_stats(path: str = "stats_10y.json") -> bool:
    """
    Charge stats_10y.json généré par HISTORICAL_STATS_ENGINE.
    Appelé au démarrage du serveur.
    Retourne True si chargé avec succès.
    """
    global _stats, _loaded, _stats_path

    # Chercher le fichier dans plusieurs emplacements
    candidates = [
        path,
        "stats_10y.json",
        "/opt/render/project/src/stats_10y.json",
        os.path.join(os.path.dirname(__file__), "stats_10y.json"),
    ]
        ...
        if os.path.exists(candidate):
            try:
                with open(candidate, "r", encoding="utf-8") as f:
                    data = json.load(f)
                _stats = data.get("symbols", {})
                _loaded = True
                _stats_path = candidate
                print(f"[HSE] stats_10y.json chargé: {len(_stats)} symboles depuis {candidate}")
                return True
            except Exception as e:
                print(f"[HSE] Erreur lecture {candidate}: {e}")

    print("[HSE] stats_10y.json absent — lancer: python HISTORICAL_STATS_ENGINE.py")
    _loaded = False
    return False
...
def get_market_hourly_bias(symbol: str, hour: int) -> dict:
    """
    Retourne le biais horaire statistique pour un symbole/heure donnés.
    Source: stats_10y.json (Yahoo Finance 7 ans + Binance + FRED)

    Retourne:
        direction:  "BUY" / "SELL" / "NEUTRAL"
        bull_rate:  float 0-1 (proportion de bougies haussières)
        confidence: float 0-1 (force du signal)
        n:          int (nombre d'échantillons)
        available:  bool (données suffisantes)
        note:       str (description)
    """
    sym = _normalize(symbol)

    if not _loaded or sym not in _stats:
        return {
            "direction":  "NEUTRAL",
            "bull_rate":  0.5,
            "confidence": 0.0,
            "n":          0,
            "available":  False,
            "note":       f"{sym} H{hour:02d}: stats_10y.json non chargé",
        }

    hs = _stats[sym].get("hour_stats", {}).get(str(hour), {})

    if not hs or hs.get("insufficient", True) or hs.get("n_samples", 0) < 30:
        return {
            "direction":  "NEUTRAL",
            "bull_rate":  0.5,
            "confidence": 0.0,
            "n":          hs.get("n_samples", 0),
            "available":  False,
            "note":       f"{sym} H{hour:02d}: données insuffisantes ({hs.get('n_samples',0)} samples)",
        }

    bull_rate  = hs.get("bull_rate", 0.5)
    direction  = hs.get("direction", "NEUTRAL")
    avg_return = hs.get("avg_return", 0.0)
    n          = hs.get("n_samples", 0)

    # Confiance = force de l'écart à 0.5 + volume de données
    vol_factor  = min(1.0, math.log(max(n, 1)) / math.log(500))
    bias_factor = abs(bull_rate - 0.5) * 2.0  # 0=neutre, 1=fort
    confidence  = round((vol_factor * 0.4 + bias_factor * 0.6), 3)
    ...
    return {
        ...
        "n":           n,
        "available":   True,
        "note":        f"{sym} H{hour:02d}: {direction} bull={bull_rate:.2f} conf={confidence:.2f} ({n} samples 7ans)",
    }


def is_transition_danger_zone(symbol: str, hour: int) -> tuple:
    """
    Retourne (is_transition: bool, info: dict | None)
    True si on est à 1h d'un changement de direction statistique important.
    Dans ce cas, réduire le lot de 20% (prudence).
    """
    sym = _normalize(symbol)
    is_trans = hour in _TRANSITION_HOURS.get(sym, set())

    if not is_trans:
        return False, None

    # Vérifier que les deux heures ont des données
    bias_now  = get_market_hourly_bias(sym, hour)
    bias_next = get_market_hourly_bias(sym, (hour + 1) % 24)

    dir_now  = bias_now.get("direction",  "NEUTRAL")
    dir_next = bias_next.get("direction", "NEUTRAL")

    # Transition réelle = directions différentes ET les deux disponibles
    real_transition = (
        bias_now["available"] and
        bias_next["available"] and
        dir_now != dir_next and
        dir_now != "NEUTRAL" and
        dir_next != "NEUTRAL"
    )
    ...
    info = {
        ...
        "dir_now":     dir_now,
        "dir_next":    dir_next,
        "real":        real_transition,
        "note":        (
            f"{sym} H{hour:02d}: transition {dir_now}→{dir_next} à H{(hour+1)%24:02d}"
            if real_transition else
            f"{sym} H{hour:02d}: zone transition mais directions identiques"
        ),
        "reduce_lot":  real_transition,
    }

    return real_transition, info


def get_status() -> dict:
    """Retourne le statut du module HSE pour /v30/status."""
    return {
        "loaded":        _loaded,
        "stats_path":    _stats_path,
        "symbols":       list(_stats.keys()),
        "n_symbols":     len(_stats),
        "transition_symbols": list(_TRANSITION_HOURS.keys()),
        "version":       "HSE-V14",
        "source":        "Yahoo Finance 7ans + Binance + FRED",
    }
