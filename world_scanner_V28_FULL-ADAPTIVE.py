# WORLD SCANNER MICRO2 ADAPTIVE — symbol-aware online source learning
# ================================================================================
...
def get_active_sessions(hour_utc: int) -> List[str]:
    ...
    london_shift = 1 if not _is_london_dst(now) else 0   # -1h en hiver GMT
    ny_shift     = 1 if not _is_ny_dst(now)     else 0   # -1h en hiver EST
    active = []
    for name, sess in SESSIONS.items():
        s, e = sess["start"], sess["end"]
        if name == "LONDON":
            s, e = s + london_shift, e + london_shift
        elif name in ("NEW_YORK", "OVERLAP_LN_NY"):
            s, e = s + ny_shift, e + ny_shift
        ...
        with _shm_lock:
            ...
            for sym, st in SHM_STATE.items():
                ...
                results[sym] = {
                    ...
                    "source":            "WORLD_SCANNER_v3.5_FULL",
