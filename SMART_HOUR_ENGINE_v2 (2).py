...
# ──────────────────────────────────────────────────────────────────────────────
# [NOUVEAU-DEMANDÉ] MICROSTRUCTURE — pondération dynamique micro vs stats 10 ans
# ──────────────────────────────────────────────────────────────────────────────
# Décidé en session : la microstructure (VPIN + patterns de bougies M1, calculés
# côté EA) a le dernier mot sur la direction quand elle est forte, mais reste
# temporée par les stats 10 ans (jamais de passage plein pot en cas de conflit
# fort — lot réduit, voir branche 8c). Poids de départ arbitraires, à calibrer
# en Phase 2 sur les trades réels via le Feature Store (régression logistique
# sur w_micro/w_hist) — ne pas les considérer comme définitifs.
W_MICRO = 0.70
W_HIST  = 0.30
MICRO_STRONG_THRESHOLD = 0.6   # |micro_score| au-delà duquel la micro a le dernier mot
...
# [FIX-STATS-08/08] Biais jour de semaine — la donnée existe dans
# stats_10y.json (weekday_stats, 14 ans, Forecaster.biz) et était déjà
# utilisée côté serveur (staline_server.py, HSE-FIX-3C) mais totalement
# absente de ce fichier, qui est pourtant celui interrogé directement par
# l'EA via get_forced_direction()/get_summary(). Corrigé : même logique
# proportionnelle à l'écart au neutre (0.5), pas un bonus/malus fixe —
# littérature (Cross 1973, French 1980, day-of-week effect) confirme lundi
# généralement pire jour / vendredi meilleur pour l'or, effet modéré mais
# répliqué sur des dizaines d'études indépendantes.
_WEEKDAY_NAMES = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]  # [BUGFIX-08/08] clés RÉELLES dans stats_10y.json (vérifié à l'exécution) — "Monday" etc. ne matchait JAMAIS, retournait toujours None silencieusement

def _get_10y_weekday_direction(sym: str, weekday_idx: int) -> Optional[str]:
    """Direction 10 ans par jour de semaine (0=lundi..6=dimanche)."""
    s = _stats10y.get(sym, {})
    wd = s.get("weekday_stats", {}).get(_WEEKDAY_NAMES[weekday_idx], {})
    strength = wd.get("strength", "WEAK")
    if wd and strength in ("MODERATE", "STRONG"):
        return wd.get("direction")
    return None


def _get_10y_weekday_bull_rate(sym: str, weekday_idx: int) -> float:
    s = _stats10y.get(sym, {})
    wd = s.get("weekday_stats", {}).get(_WEEKDAY_NAMES[weekday_idx], {})
    return wd.get("bull_rate", 0.5)
...
def smart_hour_decision(
    ...
    micro_score: Optional[float] = None,  # [NOUVEAU-DEMANDÉ] signé [-1,+1], VPIN+patterns M1 côté EA
    weekday_idx: Optional[int] = None,    # [FIX-STATS-08/08] 0=lundi..6=dimanche, None=jour UTC courant
) -> dict:
    """
    Wrapper autour de _smart_hour_decision_core() — applique en plus le biais
    jour de semaine (stats_10y.json weekday_stats, 14 ans) que ce fichier
    ignorait jusqu'ici alors qu'il est déjà utilisé côté serveur (HSE-FIX-3C).
    Ajout en wrapper plutôt qu'en modifiant _core() directement : cette
    dernière a plusieurs points de retour internes, un wrapper garantit que
    l'ajustement s'applique dans TOUS les cas sans risquer d'en oublier un.
    """
    base = _smart_hour_decision_core(symbol, hour_utc, direction, macro, hist_bias, micro_score)

    sym = _normalize(symbol)
    if weekday_idx is None:
        weekday_idx = datetime.now(timezone.utc).weekday()  # 0=lundi
    want_buy = (direction == 1)
    req_dir_str = "BUY" if want_buy else "SELL"

    wd_dir  = _get_10y_weekday_direction(sym, weekday_idx)
    wd_bull = _get_10y_weekday_bull_rate(sym, weekday_idx)
    wd_dev  = abs(wd_bull - 0.5)
    base["weekday_dir"]  = wd_dir
    base["weekday_bull"] = round(wd_bull, 3)
    base["weekday_name"] = _WEEKDAY_NAMES[weekday_idx]

    if wd_dir in ("BUY", "SELL"):
        if wd_dir == req_dir_str:
            adj = min(0.05, wd_dev * 0.7)
            base["confidence"] = min(0.99, base["confidence"] + adj)
            base["score_adj"]  = round(base.get("score_adj", 0.0) + adj, 4)
            base["reason"] += f" | +{_WEEKDAY_NAMES[weekday_idx]} confirme ({req_dir_str}, bull={wd_bull:.2f})"
        elif wd_dir != req_dir_str:
            adj = min(0.03, wd_dev * 0.4)
            base["confidence"] = max(0.01, base["confidence"] - adj)
            base["score_adj"]  = round(base.get("score_adj", 0.0) - adj, 4)
            base["reason"] += f" | -{_WEEKDAY_NAMES[weekday_idx]} contredit ({wd_dir} favori, bull={wd_bull:.2f})"

    return base


def _smart_hour_decision_core(
    symbol:    str,
    hour_utc:  int,
    direction: int,           # 1=BUY demandé, -1=SELL demandé
    macro:     Optional[dict] = None,
    ...
    - [NOUVEAU-DEMANDÉ] Si microstructure forte (|micro_score|≥0.6) confirme la
      direction demandée → interdiction de flip même si l'historique force
      l'autre sens ; lot réduit par prudence (conflit fort ≠ passage plein pot)
    """
    sym  = _normalize(symbol)
    want_buy = (direction == 1)
    ...
    base = {
        ...
        "macro_score":      0.0,
        "hist_score":       0.5,
        "micro_score":      micro_score,   # [NOUVEAU-DEMANDÉ]
        "combined_score":   None,          # [NOUVEAU-DEMANDÉ] rempli plus bas si micro_score dispo
        "macro_note":       "",
    ...
    # [NOUVEAU-DEMANDÉ] Score combiné micro/stats10y — informatif pour l'instant
    # (logué pour calibration Phase 2), n'entre pas encore dans le calcul du
    # lot/confiance directement : c'est le garde-fou explicite de la branche 8c
    # et le bonus de la branche 7 qui portent l'effet, pas ce score global seul.
    if micro_score is not None:
        base["combined_score"] = round(W_HIST * hist_score + W_MICRO * micro_score, 4)
    ...
    if direction_is_correct:
        ...
        # [NOUVEAU-DEMANDÉ] Bonus microstructure — même logique que le bonus
        # macro ci-dessus, indépendant de lui (les deux peuvent s'additionner)
        if micro_score is not None and micro_score * (1 if req_dir_str == "BUY" else -1) > MICRO_STRONG_THRESHOLD * 0.5:
            lot_boost   = min(1.5, lot_boost + 0.08)
            score_boost = min(0.10, score_boost + 0.02)
    ...
    # [NOUVEAU-DEMANDÉ] Garde microstructure — la microstructure a le dernier
    # mot sur la direction. Si micro_score est fort (≥ MICRO_STRONG_THRESHOLD)
    # ET va dans le sens de la direction demandée, interdiction ABSOLUE de
    # flipper, quelle que soit la force du signal historique (même une table
    # à grand échantillon ne doit pas écraser un signal de flux temps réel).
    # Décision explicite : conflit fort = lot réduit, pas passage plein pot,
    # pas blocage non plus — le trade part, mais prudent.
    micro_confirms_req = (
        micro_score is not None and (
            (req_dir_str == "BUY"  and micro_score >=  MICRO_STRONG_THRESHOLD) or
            (req_dir_str == "SELL" and micro_score <= -MICRO_STRONG_THRESHOLD)
        )
    )

    if force_it and micro_confirms_req:
        base.update({
            "priority":          "MEDIUM",
            "direction_changed": False,
            "force_direction":   req_dir_str,
            "final_direction":   direction,
            "lot_factor":        0.55,   # conflit fort détecté → prudence, pas plein pot
            "score_adj":         -0.03,
            "bad_dir_penalty":   0.0,
            "confidence":        0.55,
            "sources_aligned":   sources_aligned,
            "reason": (f"{sym} H{hour_utc:02d}: historique force {best_dir} (avantage ${adv}, "
                       f"{n_trades}t) mais micro_score={micro_score:+.2f} confirme fortement "
                       f"{req_dir_str} → PAS DE FLIP (micro a le dernier mot), lot×0.55 par prudence"),
            "rule": {
                "type": "micro_override_hist",
...
def get_forced_direction(symbol: str, hour_utc: int, macro: Optional[dict] = None, weekday_idx: Optional[int] = None) -> dict:
    ...
    # [FIX-STATS-08/08] Si ni l'heure réelle ni les stats 10 ans par heure ne
    # donnent de direction, on retombe sur le biais jour de semaine (14 ans) —
    # mieux qu'aucune information plutôt que de rester None.
    if weekday_idx is None:
        weekday_idx = datetime.now(timezone.utc).weekday()
    wd_dir = _get_10y_weekday_direction(sym, weekday_idx)
    if best_dir is None and wd_dir in ("BUY", "SELL"):
        best_dir = wd_dir
