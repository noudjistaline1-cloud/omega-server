...
def compute_asset_state(
    ...
    if contrarian_zone and abs(final) < 0.50:
...
BTC_WEEKDAY_STATS = {
    0: {"wr": 0.52, "label": "MON_NEUTRAL"},
    1: {"wr": 0.55, "label": "TUE_SLIGHT_BULL"},
    2: {"wr": 0.50, "label": "WED_NEUTRAL"},
    3: {"wr": 0.48, "label": "THU_SLIGHT_BEAR"},
    4: {"wr": 0.45, "label": "FRI_WEAK"},    # Vendredi = CME expiry
    5: {"wr": 0.58, "label": "SAT_BULL"},    # Weekend crypto actif
    6: {"wr": 0.55, "label": "SUN_NEUTRAL"},
}

def compute_btc_seasonality() -> Dict:
    """Calcule le biais saisonnier BTC pour la date courante."""
    ...
    w_stat = BTC_WEEKDAY_STATS.get(weekday, {"wr":0.50,"label":"UNKNOWN"})

    # Score composite : mois (70%) + jour semaine (30%)
    wr_composite = m_stat["wr"] * 0.70 + w_stat["wr"] * 0.30
    score = (wr_composite - 0.50) * 2.0  # -1.0 à +1.0

    # Effet post-halving (halving BTC : avril 2024 → bull run 12-18 mois après)
    # Avril 2024 + 14 mois = juin 2025 = pic historique attendu
    # Actuellement juin 2026 = 26 mois post-halving = phase de correction attendue
    post_halving_months = 26   # mois depuis halving avril 2024
    if 6 <= post_halving_months <= 18:
        halving_bonus = 0.20
    elif post_halving_months > 24:
        halving_bonus = -0.10   # phase correction post-bull
    else:
        halving_bonus = 0.05

    # Début/fin de mois : liquidités réallouées (effet portfolio rebalancing)
    end_of_month_effect = 0.0
    if day >= 28 or day <= 3:
        end_of_month_effect = -0.05  # pression sell fin de mois

    final_score = round(max(-1.0, min(1.0,
                       score + halving_bonus + end_of_month_effect)), 3)
        ...
        while self._running:
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                    ...
                    _seas = compute_btc_seasonality()
                    _raw_cache["seasonality"] = {"data": _seas, "ts": time.time()}
                    if _seas["signal"] != "SEASONAL_NEUTRAL":
                        updated.append(f"SEAS({_seas['signal']} {_seas['month_label']})")
